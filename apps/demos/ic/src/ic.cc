...
//Matt_S
        uint32_t mat_Time;
        uint32_t mat_oldTime = oldTime;
//matt_E

    // Initialize the platform
    NS_TRY(ns_core_init(&ns_core_cfg), "Core init failed.\n");
    NS_TRY(ns_power_config(&ns_development_default), "Power Init Failed.\n");
    NS_TRY(ns_set_performance_mode(/*NS_MINIMUM_PERF*/NS_MAXIMUM_PERF), "Set CPU Perf mode failed.");
...
#ifdef MATT_ORI_USB //Matt Add to remove ORI
...
#endif //MATT_ORI_USB //Matt Add to remove ORI
...
//Matt_S
        mat_Time = newTime - mat_oldTime;
#ifdef MATT_IO
        am_hal_gpio_state_write(NS_POWER_MONITOR_GPIO_0, AM_HAL_GPIO_OUTPUT_TOGGLE);
        am_hal_gpio_state_write(NS_POWER_MONITOR_GPIO_1, AM_HAL_GPIO_OUTPUT_TOGGLE);
#endif  //MATT_IO
        ns_lp_printf("Matt: Time taken...  %d\n us", mat_Time);
        mat_oldTime = newTime; 
//matt_E
