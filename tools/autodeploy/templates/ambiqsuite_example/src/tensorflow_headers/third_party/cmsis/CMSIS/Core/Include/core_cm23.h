...
typedef struct {
    ...
    uint32_t RESERVED1;
    __IOM uint32_t
        SHPR[2U]; /*!< Offset: 0x01C (R/W)  System Handlers Priority Registers. [0] is RESERVED */
...
typedef struct {
    ...
    __IOM uint32_t DSCSR; /*!< Offset: 0x018 (R/W)  Debug Security Control and Status Register */
        ...
        #define CoreDebug_DEMCR_DWTENA_Pos                                                         \
            24U /*!< \deprecated CoreDebug DEMCR: DWTENA Position                                  \
                 */
...
typedef struct {
    ...
    */
     ...
     */
        ...
        #else
        /*#define NVIC_SetPriorityGrouping    __NVIC_SetPriorityGrouping   not available for
         * Cortex-M23 */
        /*#define NVIC_GetPriorityGrouping    __NVIC_GetPriorityGrouping   not available for
         * Cortex-M23 */
...
__STATIC_INLINE uint32_t __NVIC_GetPriority(IRQn_Type IRQn) {
    ...
    if ((int32_t)(IRQn) >= 0) {
        ...
        return ((
            uint32_t)(((SCB->SHPR[_SHP_IDX(IRQn)] >> _BIT_SHIFT(IRQn)) & (uint32_t)0xFFUL) >> (8U - __NVIC_PRIO_BITS)));
    }
}
...
__STATIC_INLINE uint32_t TZ_NVIC_GetPriority_NS(IRQn_Type IRQn) {
    ...
    if ((int32_t)(IRQn) >= 0) {
        return ((
            uint32_t)(((NVIC_NS->IPR[_IP_IDX(IRQn)] >> _BIT_SHIFT(IRQn)) & (uint32_t)0xFFUL) >> (8U - __NVIC_PRIO_BITS)));
    } else {
        return ((
            uint32_t)(((SCB_NS->SHPR[_SHP_IDX(IRQn)] >> _BIT_SHIFT(IRQn)) & (uint32_t)0xFFUL) >> (8U - __NVIC_PRIO_BITS)));
    }
}
        #endif /*  defined (__ARM_FEATURE_CMSE) &&(__ARM_FEATURE_CMSE == 3U) */
