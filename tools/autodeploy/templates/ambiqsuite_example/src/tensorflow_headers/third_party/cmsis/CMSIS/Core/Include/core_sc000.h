...
typedef struct {
    ...
    __IOM uint32_t RASR; /*!< Offset: 0x010 (R/W)  MPU Region Attribute and Size Register */
        ...
        /**
          ...
          \brief    SC000 Core Debug Registers (DCB registers, SHCSR, and DFSR) are only accessible
          over DAP and not via processor. Therefore they are not covered by the SC000 header file.
          @{
         */
...
__STATIC_INLINE void __NVIC_SetVector(IRQn_Type IRQn, uint32_t vector) {
    ...
    /* ARM Application Note 321 states that the M0 and M0+ do not require the architectural barrier
     * - assume SC000 is the same */
}
