 ...
 */
     ...
     * \param WB Write-Back: Set to 1 to use a Write-Back policy. Set to 0 to use a Write-Through
     * policy. \param RA Read Allocation: Set to 1 to enable cache allocation on read miss. Set to 0
     * to disable cache allocation on read miss. \param WA Write Allocation: Set to 1 to enable
     * cache allocation on write miss. Set to 0 to disable cache allocation on write miss.
     */
    ...
    /**
     * Access permissions
     * AP = Access permission, RO = Read-only, RW = Read/Write, NP = Any privilege, PO = Privileged
     * code only
     */
     ...
     * \param BASE The base address bits [31:5] of a memory region. The value is zero extended.
     * Effective address gets 32 byte aligned. \param SH Defines the Shareability domain for this
     * memory region. \param RO Read-Only: Set to 1 for a read-only memory region. Set to 0 for a
     * read/write memory region. \param NP Non-Privileged: Set to 1 for a non-privileged memory
     * region. Set to 0 for privileged memory region. \param XN eXecute Never: Set to 1 for a
     * non-executable memory region. Set to 0 for an executable memory region.
     */
    ...
    #define ARM_MPU_RLAR(LIMIT, IDX)                                                               \
         ...
         * \param LIMIT The limit address bits [31:5] for this memory region. The value is one
         * extended. \param PXN Privileged execute never. Defines whether code can be executed from
         * this privileged region. \param IDX The attribute index to be associated with this memory
         * region.
         */
...
*/
__STATIC_INLINE uint32_t ARM_MPU_TYPE() { return ((MPU->TYPE) >> 8); }
...
ARM_MPU_SetRegionEx(MPU_Type *mpu, uint32_t rnr, uint32_t rbar, uint32_t rlar) {
    mpu->RNR = rnr;
    mpu->RBAR = rbar;
    mpu->RLAR = rlar;
}
 ...
 */
__STATIC_INLINE void
ARM_MPU_LoadEx(MPU_Type *mpu, uint32_t rnr, ARM_MPU_Region_t const *table, uint32_t cnt) {
