...
typedef struct {
    ...
    __IOM uint32_t
        ITCMCR; /*!< Offset: 0x10 (R/W)  Instruction Tightly-Coupled Memory Control Register */
    __IOM uint32_t DTCMCR; /*!< Offset: 0x14 (R/W)  Data Tightly-Coupled Memory Control Registers */
        ...
        #define SCB_CACR_ICACTIVE_Msk                                                              \
            (1UL << SCB_CACR_FORCEWT_Pos) /*!< SCB CACR: ICACTIVE Mask                             \
                                           */

        #define SCB_CACR_DCACTIVE_Pos 12U /*!< SCB CACR: DCACTIVE Position */
        #define SCB_CACR_DCACTIVE_Msk                                                              \
            (1UL << SCB_CACR_FORCEWT_Pos) /*!< SCB CACR: DCACTIVE Mask                             \
                                           */
...
__STATIC_INLINE uint32_t TZ_DIB_GetAuthStatus_NS(void) { return (DIB_NS->DAUTHSTATUS); }
    ...
    /*@} end of CMSIS_Core_DCBFunctions */
            ...
            #define __SCB_DCACHE_LINE_SIZE                                                         \
                32U /*!< STAR-MC1 cache line size is fixed to 32 bytes (8 words). See also         \
                       register SCB_CCSIDR */
...
__STATIC_FORCEINLINE void SCB_InvalidateICache_by_Addr(void *addr, int32_t isize) {
    ...
    if (isize > 0) {
        ...
        do {
            SCB->ICIMVAU = op_addr; /* register accepts only 32byte aligned values, only bits 31..5
                                       are valid */
...
__STATIC_FORCEINLINE void SCB_InvalidateDCache_by_Addr(void *addr, int32_t dsize) {
    ...
    if (dsize > 0) {
        ...
        do {
            SCB->DCIMVAC = op_addr; /* register accepts only 32byte aligned values, only bits 31..5
                                       are valid */
...
__STATIC_FORCEINLINE void SCB_CleanDCache_by_Addr(uint32_t *addr, int32_t dsize) {
    ...
    if (dsize > 0) {
        ...
        do {
            SCB->DCCMVAC = op_addr; /* register accepts only 32byte aligned values, only bits 31..5
                                       are valid */
            op_addr += __SCB_DCACHE_LINE_SIZE;
            op_size -= __SCB_DCACHE_LINE_SIZE;
        } while (op_size > 0);
...
/**
  ...
  invalidated. \param[in]   addr    address (aligned to 32-byte boundary) \param[in]   dsize   size
  of memory block (in number of bytes)
*/
__STATIC_FORCEINLINE void SCB_CleanInvalidateDCache_by_Addr(uint32_t *addr, int32_t dsize) {
    ...
    if (dsize > 0) {
        ...
        do {
            SCB->DCCIMVAC = op_addr; /* register accepts only 32byte aligned values, only bits 31..5
                                        are valid */
            op_addr += __SCB_DCACHE_LINE_SIZE;
            op_size -= __SCB_DCACHE_LINE_SIZE;
        } while (op_size > 0);
