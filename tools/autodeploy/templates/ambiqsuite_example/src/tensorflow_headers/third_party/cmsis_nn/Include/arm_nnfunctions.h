...
/**
   ...
   *
   * CMSIS-NN targets Cortex-M processors with typically three different implementations for each
   function. Each
   ...
   * The right implementation is picked through feature flags and the user does not have to explicit
   set it.
   *
   * \section Framework Quantization Specification
   * The library follows the [int8](https://www.tensorflow.org/lite/performance/quantization_spec)
   and int16
   ...
   *
   *
   * An example image recognition application using TensorFlow Flow Lite for Microcontrollers as an
   inference engine
   ...
   * The macros below are defined in a build system based on feature flags for a chosen processor or
   architecture
   * input to a compiler.
   * These tie in to the classification in \ref Macros.
   *
   * For a CMSIS-NN file compiled as *armclang -mcpu=cortex-m4 --target=arm-arm-none-eabi -I<CMSIS
   Core Include>
   ...
   *  Applicable when ARM_MATH_MVEI is active to let the compiler auto vectorize functions, if
   available, that uses inline
   ...
   * does not contain any non-inclusive language. If you find something that concerns you, email
   terms@arm.com.
   *
 ...
 * A collection of functions to perform basic operations for neural network layers. Functions with a
 * _s8 suffix support TensorFlow Lite framework.
 */
...
/**
 * @brief s8 convolution layer wrapper function with the main purpose to call the optimal kernel
 * available in cmsis-nn  to perform the convolution.
 *
 * @param[in, out] ctx            Function context that contains the additional buffer if required
 * by the function. arm_convolve_wrapper_s8_get_buffer_size will return the buffer_size if required.
 ...
 * @param[in]      filter_dims    Filter tensor dimensions. Format: [C_OUT, HK, WK, C_IN] where HK
 * and WK are the spatial filter dimensions
...
arm_cmsis_nn_status arm_convolve_wrapper_s8(
    ...
    const cmsis_nn_per_channel_quant_params *quant_params, const cmsis_nn_dims *input_dims,
    const int8_t *input_data, const cmsis_nn_dims *filter_dims, const int8_t *filter_data,
    const cmsis_nn_dims *bias_dims, const int32_t *bias_data, const cmsis_nn_dims *output_dims,
    int8_t *output_data);
 ...
 * @param[in]      filter_dims    Filter dimensions. Format: [C_OUT, HK, WK, C_IN] where HK and WK
...
int32_t arm_convolve_wrapper_s8_get_buffer_size(
    const cmsis_nn_conv_params *conv_params, const cmsis_nn_dims *input_dims,
    const cmsis_nn_dims *filter_dims, const cmsis_nn_dims *output_dims);

/**
 * @brief Get the required buffer size for arm_convolve_wrapper_s8 for Arm(R) Helium Architecture
 * case. Refer to arm_convolve_wrapper_s8_get_buffer_size() for function argument details.
 *
...
/**
 * @brief Get the required buffer size for arm_convolve_wrapper_s8 for processors with DSP
 * extension. Refer to arm_convolve_wrapper_s8_get_buffer_size() for function argument details.
 *
 * @note       Intended for compilation on Host. If compiling for an Arm target, use
 *             arm_convolve_wrapper_s8_get_buffer_size().
 *
...
/**
 * @brief s16 convolution layer wrapper function with the main purpose to call the optimal kernel
 * available in cmsis-nn to perform the convolution.
 *
 * @param[in, out] ctx            Function context that contains the additional buffer if required
 * by the function. arm_convolve_wrapper_s8_get_buffer_size will return the buffer_size if required
...
arm_cmsis_nn_status arm_convolve_wrapper_s16(
    const cmsis_nn_context *ctx, const cmsis_nn_conv_params *conv_params,
    const cmsis_nn_per_channel_quant_params *quant_params, const cmsis_nn_dims *input_dims,
    const int16_t *input_data, const cmsis_nn_dims *filter_dims, const int8_t *filter_data,
    const cmsis_nn_dims *bias_dims, const int64_t *bias_data, const cmsis_nn_dims *output_dims,
    int16_t *output_data);
...
/**
 * @brief Get the required buffer size for arm_convolve_wrapper_s16 for for processors with DSP
 * extension. Refer to arm_convolve_wrapper_s16_get_buffer_size() for function argument details.
 *
 * @note       Intended for compilation on Host. If compiling for an Arm target, use
...
/**
 * @brief Get the required buffer size for arm_convolve_wrapper_s16 for Arm(R) Helium Architecture
 * case. Refer to arm_convolve_wrapper_s16_get_buffer_size() for function argument details.
 *
 * @note       Intended for compilation on Host. If compiling for an Arm target, use
 *             arm_convolve_wrapper_s16_get_buffer_size().
 *
 ...
 * @param[in, out] ctx            Function context that contains the additional buffer if required
 by the function.
 *                                arm_convolve_s8_get_buffer_size will return the buffer_size if
 required.
 *                                The caller is expected to clear the buffer ,if applicable, for
 security reasons.
 * @param[in]      conv_params    Convolution parameters (e.g. strides, dilations, pads,...).
 ...
 *
 * @param[in]       input_dims            Input (activation) tensor dimensions. Format: [N, H, W,
 * C_IN]
 * @param[in]       filter_dims           Filter tensor dimensions. Format: [C_OUT, HK, WK, C_IN]
 * where HK and WK are the spatial filter dimensions
 * @return          The function returns required buffer size(bytes)
 *
 */
int32_t
arm_convolve_s8_get_buffer_size(const cmsis_nn_dims *input_dims, const cmsis_nn_dims *filter_dims);

/**
 * @brief Basic s16 convolution function
 * @param[in, out] ctx            Function context that contains the additional buffer if required
 by the function.
 *                                arm_convolve_s16_get_buffer_size will return the buffer_size if
 required.
 *                                The caller is expected to clear the buffer ,if applicable, for
 security reasons.
 * @param[in]      conv_params    Convolution parameters (e.g. strides, dilations, pads,...).
 ...
 * @param[in, out] ctx            Function context that contains the additional buffer if required
 by the function.
 *                                arm_convolve_fast_s16_get_buffer_size will return the buffer_size
 if required.
 *                                The caller is expected to clear the buffer ,if applicable, for
 security reasons.
 * @param[in]      conv_params    Convolution parameters (e.g. strides, dilations, pads,...).
 ...
 * @param[in]      filter_dims    Filter tensor dimensions. Format: [C_OUT, HK, WK, C_IN] where HK
 and WK are the
 *                                spatial filter dimensions. (filter_dims->w * filter_dims->h *
 input_dims->c) must not exceed 512
 * @param[in]      filter_data    Filter data pointer. Data type: int8
 * @param[in]      bias_dims      Bias tensor dimensions. Format: [C_OUT]
 * @param[in]      bias_data      Optional bias data pointer. Data type: int64
 * @param[in]      output_dims    Output tensor dimensions. Format: [N, H, W, C_OUT]
 ...
 *    1. Supported framework: TensorFlow Lite micro
 *    2. Additional memory is required for optimization. Refer to argument 'ctx' for details.
 *    3. Implementation supports kernel volumes (filter width * filter height * input channels) <
 512.
 *
 */

arm_cmsis_nn_status arm_convolve_fast_s16(
    const cmsis_nn_context *ctx, const cmsis_nn_conv_params *conv_params,
    const cmsis_nn_per_channel_quant_params *quant_params, const cmsis_nn_dims *input_dims,
    const int16_t *input_data, const cmsis_nn_dims *filter_dims, const int8_t *filter_data,
    const cmsis_nn_dims *bias_dims, const int64_t *bias_data, const cmsis_nn_dims *output_dims,
 ...
 */
int32_t
arm_convolve_s16_get_buffer_size(const cmsis_nn_dims *input_dims, const cmsis_nn_dims *filter_dims);
 ...
 *
 */
int32_t arm_convolve_fast_s16_get_buffer_size(
    const cmsis_nn_dims *input_dims, const cmsis_nn_dims *filter_dims);

/**
 * @brief Fast s8 version for 1x1 convolution (non-square shape)
 *
 * @param[in, out] ctx           Function context that contains the additional buffer if required by
 * the function. arm_convolve_1x1_s8_fast_get_buffer_size will return the buffer_size if required.
 *                               The caller is expected to clear the buffer ,if applicable, for
 * security reasons.
 * @param[in]      conv_params   Convolution parameters (e.g. strides, dilations, pads,...).
 *                               Range of conv_params->input_offset  : [-127, 128]
 ...
 *
 * @param[in, out] ctx           Function context that contains the additional buffer if required by
 * the function. None is required by this function.
 * @param[in]      conv_params   Convolution parameters (e.g. strides, dilations, pads,...).
 *                               Range of conv_params->input_offset  : [-127, 128]
 *                               Range of conv_params->output_offset : [-128, 127]
 * @param[in]      quant_params  Per-channel quantization info.
 ...
 *
 * @param[in, out] ctx           Function context that contains the additional buffer if required by
 *the function. arm_convolve_1_x_n_s8_get_buffer_size will return the buffer_size if required The
 *caller is expected to clear the buffer ,if applicable, for security reasons.
 * @param[in]      conv_params   Convolution parameters (e.g. strides, dilations, pads,...).
 *                               Range of conv_params->input_offset  : [-127, 128]
 *                               Range of conv_params->output_offset : [-128, 127]
 * @param[in]      quant_params  Per-channel quantization info.
 ...
 * @param[in]      input_dims    Input (activation) tensor dimensions. Format: [N, H, W, C_IN]
 * @param[in]      input_data    Input (activation) data pointer. Data type: int8
 * @param[in]      filter_dims   Filter tensor dimensions. Format: [C_OUT, 1, WK, C_IN] where WK is
 *the horizontal spatial filter dimension
 * @param[in]      filter_data   Filter data pointer. Data type: int8
 * @param[in]      bias_dims     Bias tensor dimensions. Format: [C_OUT]
 * @param[in]      bias_data     Optional bias data pointer. Data type: int32
 * @param[in]      output_dims   Output tensor dimensions. Format: [N, H, W, C_OUT]
 ...
 * @param[in]       input_dims            Input (activation) tensor dimensions. Format: [N, H, W,
 * C_IN]
 * @param[in]       filter_dims           Filter tensor dimensions. Format: [C_OUT, 1, WK, C_IN]
 * where WK is the horizontal spatial filter dimension
 * @return          The function returns required buffer size(bytes)
 *
 */
 ...
 *                                 The caller is expected to clear the buffer ,if applicable, for
 * security reasons.
 * @param[in]      dw_conv_params  Depthwise convolution parameters (e.g. strides, dilations,
 * pads,...) dw_conv_params->dilation is not used. Range of dw_conv_params->input_offset : [-127,
 * 128] Range of dw_conv_params->output_offset : [-128, 127]
 * @param[in]      quant_params    Per-channel quantization info.
 *                                 It contains the multiplier and shift values to be applied to each
 *                                 output channel
 ...
 *    - Check details of arm_depthwise_conv_s8_opt() for potential data that can be accessed outside
 * of the boundary.
 */
arm_cmsis_nn_status arm_depthwise_conv_wrapper_s8(
    const cmsis_nn_context *ctx, const cmsis_nn_dw_conv_params *dw_conv_params,
    const cmsis_nn_per_channel_quant_params *quant_params, const cmsis_nn_dims *input_dims,
    const int8_t *input_data, const cmsis_nn_dims *filter_dims, const int8_t *filter_data,
    const cmsis_nn_dims *bias_dims, const int32_t *bias_data, const cmsis_nn_dims *output_dims,
    int8_t *output_data);
 ...
 *
 * @param[in]      dw_conv_params  Depthwise convolution parameters (e.g. strides, dilations,
 * pads,...) Range of dw_conv_params->input_offset : [-127, 128] Range of
 * dw_conv_params->input_offset : [-128, 127]
 * @param[in]      input_dims      Input (activation) tensor dimensions. Format: [H, W, C_IN]
 *                                 Batch argument N is not used and assumed to be 1.
 * @param[in]      filter_dims     Filter tensor dimensions. Format: [1, H, W, C_OUT]
 * @param[in]      output_dims     Output tensor dimensions. Format: [1, H, W, C_OUT]
...
int32_t arm_depthwise_conv_wrapper_s8_get_buffer_size(
    const cmsis_nn_dw_conv_params *dw_conv_params, const cmsis_nn_dims *input_dims,
    const cmsis_nn_dims *filter_dims, const cmsis_nn_dims *output_dims);

/**
 * @brief Get size of additional buffer required by arm_depthwise_conv_wrapper_s8() for processors
 * with DSP extension. Refer to arm_depthwise_conv_wrapper_s8_get_buffer_size() for function
 * argument details.
 *
 * @note       Intended for compilation on Host. If compiling for an Arm target, use
...
/**
 * @brief Get size of additional buffer required by arm_depthwise_conv_wrapper_s8() for Arm(R)
 * Helium Architecture case. Refer to arm_depthwise_conv_wrapper_s8_get_buffer_size() for function
 * argument details.
 *
 * @note       Intended for compilation on Host. If compiling for an Arm target, use
 *             arm_depthwise_conv_wrapper_s8_get_buffer_size().
...
/**
 * @brief Basic s8 depthwise convolution function that doesn't have any constraints on the input
 * dimensions.
 *
 * @param[in, out] ctx             Function context (e.g. temporary buffer). Check the function
 *                                 definition file to see if an additional buffer is required.
 *                                 Optional function {API}_get_buffer_size() provides the buffer
 *                                 size if an additional buffer is required exists if additional
 * memory is. The caller is expected to clear the buffer ,if applicable, for security reasons.
 * @param[in]      dw_conv_params  Depthwise convolution parameters (e.g. strides, dilations,
 * pads,...) dw_conv_params->dilation is not used. Range of dw_conv_params->input_offset : [-127,
 * 128] Range of dw_conv_params->input_offset : [-128, 127]
 * @param[in]      quant_params    Per-channel quantization info.
 *                                 It contains the multiplier and shift values to be applied to each
 *                                 output channel
 * @param[in]      input_dims      Input (activation) tensor dimensions. Format: [N, H, W, C_IN]
...
/**
 * @brief Basic s16 depthwise convolution function that doesn't have any constraints on the input
 * dimensions.
 *
 * @param[in, out] ctx             Function context (e.g. temporary buffer). Check the function
 *                                 definition file to see if an additional buffer is required.
 ...
 * security reasons.
 * @param[in]      dw_conv_params  Depthwise convolution parameters (e.g. strides, dilations,
 * pads,...) conv_params->input_offset  : Not used conv_params->output_offset : Not used
 * @param[in]      quant_params    Per-channel quantization info.
 *                                 It contains the multiplier and shift values to be applied to each
 *                                 output channel
 * @param[in]      input_dims      Input (activation) tensor dimensions. Format: [N, H, W, C_IN]
 ...
 * security reasons.
 * @param[in]      dw_conv_params  Depthwise convolution parameters (e.g. strides, dilations,
 * pads,...) dw_conv_params->dilation is not used. Range of dw_conv_params->input_offset : Not used
 ...
 *
 * @param[in]      dw_conv_params  Depthwise convolution parameters (e.g. strides, dilations,
 * pads,...) Range of dw_conv_params->input_offset : Not used Range of dw_conv_params->input_offset
 * : Not used
 * @param[in]      input_dims      Input (activation) tensor dimensions. Format: [H, W, C_IN]
 *                                 Batch argument N is not used and assumed to be 1.
 * @param[in]      filter_dims     Filter tensor dimensions. Format: [1, H, W, C_OUT]
...
/**
 * @brief Get size of additional buffer required by arm_depthwise_conv_wrapper_s16() for processors
 * with DSP extension. Refer to arm_depthwise_conv_wrapper_s16_get_buffer_size() for function
 * argument details.
 *
 * @note       Intended for compilation on Host. If compiling for an Arm target, use
...
/**
 * @brief Get size of additional buffer required by arm_depthwise_conv_wrapper_s16() for Arm(R)
 * Helium Architecture case. Refer to arm_depthwise_conv_wrapper_s16_get_buffer_size() for function
 * argument details.
 *
 * @note       Intended for compilation on Host. If compiling for an Arm target, use
 *             arm_depthwise_conv_wrapper_s16_get_buffer_size().
...
/**
 * @brief Optimized s16 depthwise convolution function with constraint that in_channel equals
 * out_channel. Refer arm_depthwise_conv_s16() for function argument details.
 *
 * @return     The function returns one of the following
 *                <code>ARM_CMSIS_NN_ARG_ERROR</code> - ctx-buff == NULL and
 *                                                      arm_depthwise_conv_fast_s16_get_buffer_size()
 * > 0 or input channel != output channel or ch_mult != 1
 *
 ...
 *
 */
int32_t arm_depthwise_conv_fast_s16_get_buffer_size(
    const cmsis_nn_dims *input_dims, const cmsis_nn_dims *filter_dims);
...
/**
 * @brief Optimized s8 depthwise convolution function with constraint that in_channel equals
 * out_channel. Refer arm_depthwise_conv_s8() for function argument details.
 *
 * @return     The function returns one of the following
 ...
 *                <code>ARM_CMSIS_NN_SUCCESS</code> - Successful operation
 *
 * @note       If number of channels is not a multiple of 4, upto 3 elements outside the boundary
 * will be read out for the following if MVE optimizations(Arm Helium Technology) are used.
 *               - Output shift
 *               - Output multiplier
 *               - Output bias
 *               - kernel
 ...
 *                               It contains the multiplier and shift values to be applied to the
 * output tensor.
 * @param[in]      input_dims    Input (activation) tensor dimensions. Format: [N, H, W, C_IN]
...
arm_cmsis_nn_status arm_fully_connected_s8(
    ...
    const cmsis_nn_per_tensor_quant_params *quant_params, const cmsis_nn_dims *input_dims,
    const int8_t *input_data, const cmsis_nn_dims *filter_dims, const int8_t *filter_data,
    const cmsis_nn_dims *bias_dims, const int32_t *bias_data, const cmsis_nn_dims *output_dims,
    int8_t *output_data);
...
/**
 * @brief Get size of additional buffer required by arm_fully_connected_s8() for processors with DSP
 * extension. Refer to arm_fully_connected_s8_get_buffer_size() for function argument details.
 *
 * @note       Intended for compilation on Host. If compiling for an Arm target, use
...
/**
 * @brief Get size of additional buffer required by arm_fully_connected_s8() for Arm(R) Helium
 * Architecture case. Refer to arm_fully_connected_s8_get_buffer_size() for function argument
 * details.
 *
 * @note       Intended for compilation on Host. If compiling for an Arm target, use
 *             arm_fully_connected_s8_get_buffer_size().
...
/**
 * @brief Get size of additional buffer required by arm_fully_connected_s16() for processors with
 * DSP extension. Refer to arm_fully_connected_s16_get_buffer_size() for function argument details.
 *
 * @note       Intended for compilation on Host. If compiling for an Arm target, use
...
/**
 * @brief Get size of additional buffer required by arm_fully_connected_s16() for Arm(R) Helium
 * Architecture case. Refer to arm_fully_connected_s16_get_buffer_size() for function argument
 * details.
 *
 * @note       Intended for compilation on Host. If compiling for an Arm target, use
 *             arm_fully_connected_s16_get_buffer_size().
...
arm_cmsis_nn_status arm_elementwise_add_s8(
    const int8_t *input_1_vect, const int8_t *input_2_vect, const int32_t input_1_offset,
    const int32_t input_1_mult, const int32_t input_1_shift, const int32_t input_2_offset,
    const int32_t input_2_mult, const int32_t input_2_shift, const int32_t left_shift,
...
arm_cmsis_nn_status arm_elementwise_add_s16(
    ...
    const int32_t input_1_mult, const int32_t input_1_shift, const int32_t input_2_offset,
    const int32_t input_2_mult, const int32_t input_2_shift, const int32_t left_shift,
    int16_t *output, const int32_t out_offset, const int32_t out_mult, const int32_t out_shift,
    const int32_t out_activation_min, const int32_t out_activation_max, const int32_t block_size);
...
arm_cmsis_nn_status arm_elementwise_mul_s8(
    const int8_t *input_1_vect, const int8_t *input_2_vect, const int32_t input_1_offset,
    const int32_t input_2_offset, int8_t *output, const int32_t out_offset, const int32_t out_mult,
 ...
 * This activation function must be bit precise congruent with the corresponding TFLM tanh and
 * sigmoid actication functions
 */
void arm_nn_activation_s16(
    const int16_t *input, int16_t *output, const uint16_t size, const uint16_t left_shift,
...
/**
 * @brief Get the required buffer size for S8 average pooling function for processors with DSP
 * extension. Refer to arm_avgpool_s8_get_buffer_size() for function argument details.
 *
 * @note       Intended for compilation on Host. If compiling for an Arm target, use
...
/**
 * @brief Get the required buffer size for S8 average pooling function for Arm(R) Helium
 * Architecture case. Refer to arm_avgpool_s8_get_buffer_size() for function argument details.
 *
 * @note       Intended for compilation on Host. If compiling for an Arm target, use
 *             arm_avgpool_s8_get_buffer_size().
 *
 ...
 * @return                        The function returns
 *                                    <code>ARM_CMSIS_NN_SUCCESS</code> - Successful operation
 *                                    <code>ARM_CMSIS_NN_ARG_ERROR</code> - In case of invalid
 * arguments
 *
 * @details
 *    - Supported Framework: TensorFlow Lite
...
arm_cmsis_nn_status arm_avgpool_s16(
    const cmsis_nn_context *ctx, const cmsis_nn_pool_params *pool_params,
    const cmsis_nn_dims *input_dims, const int16_t *input_data, const cmsis_nn_dims *filter_dims,
    const cmsis_nn_dims *output_dims, int16_t *output_data);
...
/**
 * @brief Get the required buffer size for S16 average pooling function for processors with DSP
 * extension. Refer to arm_avgpool_s16_get_buffer_size() for function argument details.
 *
 * @note       Intended for compilation on Host. If compiling for an Arm target, use
...
/**
 * @brief Get the required buffer size for S16 average pooling function for Arm(R) Helium
 * Architecture case. Refer to arm_avgpool_s16_get_buffer_size() for function argument details.
 *
 * @note       Intended for compilation on Host. If compiling for an Arm target, use
 *             arm_avgpool_s16_get_buffer_size().
 *
...
arm_cmsis_nn_status arm_max_pool_s16(
    const cmsis_nn_context *ctx, const cmsis_nn_pool_params *pool_params,
    const cmsis_nn_dims *input_dims, const int16_t *src, const cmsis_nn_dims *filter_dims,
    const cmsis_nn_dims *output_dims, int16_t *dst);
 ...
 */
void arm_softmax_s8(
    const int8_t *input, const int32_t num_rows, const int32_t row_size, const int32_t mult,
    const int32_t shift, const int32_t diff_min, int8_t *output);
...
void arm_softmax_s8_s16(
    const int8_t *input, const int32_t num_rows, const int32_t row_size, const int32_t mult,
    const int32_t shift, const int32_t diff_min, int16_t *output);
 ...
 * @param[in]  mult            Input quantization multiplier
 * @param[in]  shift           Input quantization shift within the range [0, 31]
 * @param[in]  softmax_params  Softmax s16 layer parameters with two pointers to LUTs speficied
 * below. For indexing the high 9 bits are used and 7 remaining for interpolation. That means 512
 * entries for the 9-bit indexing and 1 extra for interpolation, i.e. 513 values for each LUT.
 *                             - Lookup table for exp(x), where x uniform distributed between [-10.0
 * , 0.0]
 *                             - Lookup table for 1 / (1 + x), where x uniform distributed between
 * [0.0 , 1.0]
 * @param[out] output          Pointer to the output tensor
 * @return                        The function returns
 *                                    <code>ARM_CMSIS_NN_ARG_ERROR</code> Argument error check
 * failed <code>ARM_CMSIS_NN_SUCCESS</code> - Successful operation
 *
 * @note Supported framework: TensorFlow Lite micro (bit-accurate)
 *
 */
arm_cmsis_nn_status arm_softmax_s16(
    const int16_t *input, const int32_t num_rows, const int32_t row_size, const int32_t mult,
    const int32_t shift, const cmsis_nn_softmax_lut_s16 *softmax_params, int16_t *output);
...
void arm_softmax_u8(
    const uint8_t *input, const int32_t num_rows, const int32_t row_size, const int32_t mult,
    const int32_t shift, const int32_t diff_min, uint8_t *output);
 ...
 *
 * @note This function, data layout independent, can be used to concatenate either int8 or uint8
 * tensors because it does not involve any arithmetic operation
 *
 * @param[in]  input    Pointer to input tensor. Input tensor must not overlap with the output
 * tensor.
 ...
 * @param[in]  offset_x The offset (in number of elements) on the X axis to start concatenating the
 * input tensor It is user responsibility to provide the correct value
 *
...
void arm_concatenation_s8_x(
    const int8_t *input, const uint16_t input_x, const uint16_t input_y, const uint16_t input_z,
    const uint16_t input_w, int8_t *output, const uint16_t output_x, const uint32_t offset_x);
...
void arm_concatenation_s8_y(
    const int8_t *input, const uint16_t input_x, const uint16_t input_y, const uint16_t input_z,
    const uint16_t input_w, int8_t *output, const uint16_t output_y, const uint32_t offset_y);
...
void arm_concatenation_s8_z(
    const int8_t *input, const uint16_t input_x, const uint16_t input_y, const uint16_t input_z,
    const uint16_t input_w, int8_t *output, const uint16_t output_z, const uint32_t offset_z);

/**
 * @brief int8/uint8 concatenation function to be used for concatenating N-tensors along the W axis
 * (Batch size) This function should be called for each input tensor to concatenate. The argument
 * offset_w will be used to store the input tensor in the correct position in the output tensor
 *
 *        i.e.    offset_w = 0
 *                for(i = 0 i < num_input_tensors; ++i)
 *                {
...
void arm_concatenation_s8_w(
    const int8_t *input, const uint16_t input_x, const uint16_t input_y, const uint16_t input_z,
    const uint16_t input_w, int8_t *output, const uint32_t offset_w);
/**
...
arm_cmsis_nn_status arm_svdf_state_s16_s8(
    ...
    const cmsis_nn_dims *weights_feature_dims, const int8_t *weights_feature_data,
    const cmsis_nn_dims *weights_time_dims, const int16_t *weights_time_data,
    const cmsis_nn_dims *bias_dims, const int32_t *bias_data, const cmsis_nn_dims *output_dims,
    int8_t *output_data);
 ...
 * Peephole connections, projection, clipping, combined input/forget gate and layer normalization
 * are not supported.
 *
 ...
 *
 *       1 Input to input weight can not be nullptr. Otherwise nullptr for combined input/forgat
 * gate. 2 Cell weights are not used and should be nullptr. Otherwise needed for peephole
 * connections. 3 Projection weight is not used and should be nullpr. Otherwise needed for
 * projection.
 *
 * @return     The function returns <code>ARM_CMSIS_NN_SUCCESS</code>
 *
 ...
 *
 */
arm_cmsis_nn_status arm_lstm_unidirectional_s16_s8(
    cmsis_nn_lstm_context *scratch_buffers, const int8_t *input_data,
    const cmsis_nn_lstm_dims *lstm_dims, const int8_t *input_to_input_weights,
    const int8_t *input_to_forget_weights, const int8_t *input_to_cell_weights,
    const int8_t *input_to_output_weights, const int8_t *recurrent_to_input_weights,
    const int8_t *recurrent_to_forget_weights, const int8_t *recurrent_to_cell_weights,
    const int8_t *recurrent_to_output_weights, const int16_t *cell_to_input_weights,
    const int16_t *cell_to_forget_weights, const int16_t *cell_to_output_weights,
    const int8_t *projection_weights, const cmsis_nn_lstm_params *lstm, int8_t *output_state,
    int16_t *cell_state, int8_t *output_data);
