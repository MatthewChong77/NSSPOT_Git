...
class Label {
  public:
    Label() {}
    template <typename... Args> explicit Label(Args... args) { Set(args...); }
    void Set(const char *format) {
        format_ = format;
        args_count_ = 0;
    }
    template <typename... Args> void Set(const char *format, Args... args) {
        format_ = format;
  ...
  private:
    ...
    template <typename... Args> void SetArgs(int position, int arg0, Args... args) {
