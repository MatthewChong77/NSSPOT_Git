...
class Verifier FLATBUFFERS_FINAL_CLASS {
  public:
    Verifier(
        const uint8_t *const buf, const size_t buf_len, const uoffset_t _max_depth = 64,
        const uoffset_t _max_tables = 1000000, const bool _check_alignment = true)
        : buf_(buf), size_(buf_len), max_depth_(_max_depth), max_tables_(_max_tables),
          check_alignment_(_check_alignment), upper_bound_(0), depth_(0), num_tables_(0),
    ...
    template <typename T>
    bool
    VerifyField(const uint8_t *const base, const voffset_t elem_off, const size_t align) const {
        const auto f = static_cast<size_t>(base - buf_) + elem_off;
    ...
    template <typename T> bool VerifyVector(const Vector<const T *> *const vec) const {
    ...
    bool VerifyVectorOrString(
        const uint8_t *const vec, const size_t elem_size, size_t *const end = nullptr) const {
    ...
    template <typename T> bool VerifyVectorOfTables(const Vector<Offset<T>> *const vec) {
        if (vec) {
            for (uoffset_t i = 0; i < vec->size(); i++) {
    ...
    bool VerifyBufferFromStart(const char *const identifier, const size_t start) {
        ...
        if (!Check(size_ >= FLATBUFFERS_MIN_BUFFER_SIZE))
            return false;
        ...
        return Check(o != 0) && reinterpret_cast<const T *>(buf_ + start + o)->Verify(*this)
        // clang-format off
    #ifdef FLATBUFFERS_TRACK_VERIFIER_BUFFER_SIZE
           && GetComputedSize()
    #endif
    ...
    template <typename T>
    bool VerifyNestedFlatBuffer(const Vector<uint8_t> *const buf, const char *const identifier) {
        // An empty buffer is OK as it indicates not present.
        if (!buf)
            return true;

        // If there is a nested buffer, it must be greater than the min size.
        if (!Check(buf->size() >= FLATBUFFERS_MIN_BUFFER_SIZE))
            return false;
    ...
    template <typename T> bool VerifySizePrefixedBuffer(const char *const identifier) {
    ...
    uoffset_t VerifyOffset(const uint8_t *const base, const voffset_t start) const {
