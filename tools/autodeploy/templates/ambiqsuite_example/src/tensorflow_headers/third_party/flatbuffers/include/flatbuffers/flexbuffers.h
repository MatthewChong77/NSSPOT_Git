...
class Map : public Vector {
  public:
    ...
    TypedVector Keys() const {
        ...
        return TypedVector(
            Indirect(keys_offset, byte_width_),
            static_cast<uint8_t>(ReadUInt64(keys_offset + byte_width_, byte_width_)), FBT_KEY);
    }
...
class Reference {
  public:
    Reference() : data_(nullptr), parent_width_(0), byte_width_(0), type_(FBT_NULL) {}
    ...
    void ToString(bool strings_quoted, bool keys_quoted, std::string &s) const {
        if (type_ == FBT_STRING) {
            ...
            flatbuffers::EscapeString(
                reinterpret_cast<const char *>(blob.data()), blob.size(), &s, true, false);
        } else {
            s += "(?)";
        }
    }
...
class Builder FLATBUFFERS_FINAL_CLASS {
  public:
    ...
    template <typename T> void Vector(const std::vector<T> &vec) { Vector(vec.data(), vec.size()); }
  ...
  private:
    ...
    void WriteBytes(const void *val, size_t size) {
        buf_.insert(
            buf_.end(), reinterpret_cast<const uint8_t *>(val),
  ...
  public:
    ...
    struct Value {
        ...
        Value(bool b)
            : u_(static_cast<uint64_t>(b)), type_(FBT_BOOL), min_bit_width_(BIT_WIDTH_8) {}
        ...
        Value(float f)
            : f_(static_cast<double>(f)), type_(FBT_FLOAT), min_bit_width_(BIT_WIDTH_32) {}
        ...
        BitWidth ElemWidth(size_t buf_size, size_t elem_index) const {
            if (IsInline(type_)) {
                ...
                for (size_t byte_width = 1; byte_width <= sizeof(flatbuffers::largest_scalar_t);
                    ...
                    auto offset_loc = buf_size + flatbuffers::PaddingBytes(buf_size, byte_width) +
  ...
  private:
    ...
    template <typename T> size_t ScalarVector(const T *elems, size_t len, bool fixed) {
        ...
        stack_.push_back(Value(
            static_cast<uint64_t>(vloc), ToTypedVector(vector_type, fixed ? len : 0), bit_width));
        return vloc;
    }

    Value CreateVector(
        size_t start, size_t vec_len, size_t step, bool typed, bool fixed,
        const Value *keys = nullptr) {
        FLATBUFFERS_ASSERT(
            !fixed || typed); // typed=false, fixed=true combination is not supported.
        ...
        return Value(
            static_cast<uint64_t>(vloc),
            keys ? FBT_MAP : (typed ? ToTypedVector(vector_type, fixed ? vec_len : 0) : FBT_VECTOR),
            bit_width);
    }
...
class Verifier FLATBUFFERS_FINAL_CLASS {
  public:
    Verifier(
        ...
        std::vector<uint8_t> *reuse_tracker = nullptr, bool _check_alignment = true,
        size_t max_depth = 64)
        : buf_(buf), size_(buf_len), depth_(0), max_depth_(max_depth), num_vectors_(0),
          max_vectors_(buf_len), check_alignment_(_check_alignment), reuse_tracker_(reuse_tracker) {
  ...
  private:
    ...
    bool VerifyBefore(size_t elem, size_t elem_len) const { return Check(elem_len <= elem); }
    ...
    bool VerifyOffset(uint64_t off, const uint8_t *p) {
        return Check(off <= static_cast<uint64_t>(size_)) && off <= static_cast<uint64_t>(p - buf_);
    }
...
#define FLEX_CHECK_VERIFIED(P, PACKED_TYPE)                                                        \
    ...
    bool VerifyVector(Reference r, const uint8_t *p, Type elem_type) {
        ...
        if (!VerifyBeforePointer(p, size_byte_width))
            return false;
        FLEX_CHECK_VERIFIED(
            p - size_byte_width, PackedType(Builder::WidthB(size_byte_width), r.type_));
    ...
    bool VerifyKeys(const uint8_t *p, uint8_t byte_width) {
        ...
        if (!VerifyOffset(off, p))
            return false;
        auto key_byte_with = static_cast<uint8_t>(ReadUInt64(p + byte_width, byte_width));
        if (!VerifyByteWidth(key_byte_with))
            return false;
        return VerifyVector(
            Reference(p, byte_width, key_byte_with, FBT_VECTOR_KEY), p - off, FBT_KEY);
    }
...
#undef FLEX_CHECK_VERIFIED
    ...
    bool VerifyTerminator(const String &s) {
        return VerifyFromPointer(reinterpret_cast<const uint8_t *>(s.c_str()), s.size() + 1);
    }
    ...
    bool VerifyRef(Reference r) {
        ...
        case FBT_BLOB:
            return VerifyVector(r, p, FBT_UINT);
        case FBT_STRING:
            return VerifyVector(r, p, FBT_UINT) && VerifyTerminator(String(p, r.byte_width_));
...
// parameters.
inline bool
VerifyBuffer(const uint8_t *buf, size_t buf_len, std::vector<uint8_t> *reuse_tracker = nullptr) {
