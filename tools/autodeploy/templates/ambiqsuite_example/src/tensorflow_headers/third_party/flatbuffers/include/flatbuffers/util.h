...
template <typename T>
inline bool StringToIntegerImpl(
    T *val, const char *const str, const int base = 0, const bool check_errno = true) {
...
// line, respectively.
inline std::string WordWrap(
    const std::string in, size_t max_length, const std::string wrapped_line_prefix,
...
inline bool EscapeString(
    const char *s, size_t length, std::string *_text, bool allow_non_utf8, bool natural_utf8) {
...
inline std::string BufferToHexText(
    const void *buffer, size_t buffer_size, size_t max_length,
    const std::string &wrapped_line_prefix, const std::string &wrapped_line_suffix) {
