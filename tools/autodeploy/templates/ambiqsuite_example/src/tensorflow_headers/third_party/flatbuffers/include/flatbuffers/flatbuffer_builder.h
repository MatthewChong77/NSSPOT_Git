...
template <typename T, typename Alloc = std::allocator<T>> T *data(std::vector<T, Alloc> &v) {
    // Eventually the returned pointer gets passed down to memcpy, so
    // we need it to be non-null to avoid undefined behavior.
    static uint8_t t;
...
class FlatBufferBuilder {
  public:
    ...
    explicit FlatBufferBuilder(
        size_t initial_size = 1024, Allocator *allocator = nullptr, bool own_allocator = false,
    ...
    FlatBufferBuilder(FlatBufferBuilder &&other)
        : buf_(1024, nullptr, false, AlignOf<largest_scalar_t>()), num_field_loc(0),
          max_voffset_(0), nested(false), finished(false), minalign_(1), force_defaults_(false),
          dedup_vtables_(true), string_pool(nullptr) {
        EndianCheck();
    ...
    uoffset_t EndTable(uoffset_t start) {
        ...
        max_voffset_ = (std::max)(
            static_cast<voffset_t>(max_voffset_ + sizeof(voffset_t)), FieldIndexToOffset(0));
  ...
  Offset<String> CreateString(flatbuffers::string_view str) {
    ...
    /// @return Returns a typed `Offset` into the serialized data indicating
    /// where the vector is stored.
    template <typename T> Offset<Vector<T>> CreateVector(std::initializer_list<T> v) {
    ...
    /// @return Returns a typed `Offset` into the serialized data indicating
    /// where the vector is stored.
    template <class It> Offset<Vector<Offset<String>>> CreateVectorOfStrings(It begin, It end) {
    ...
    CreateVectorOfNativeStructs(const S *v, size_t len, T (*const pack_func)(const S &)) {
    ...
    template <typename T>
    Offset<Vector<const T *>>
    CreateVectorOfStructs(size_t vector_size, const std::function<void(size_t i, T *)> &filler) {
    ...
    Offset<Vector<const T *>>
    CreateVectorOfNativeStructs(const std::vector<S, Alloc> &v, T (*const pack_func)(const S &)) {
  ...
  protected:
    ...
    void Finish(uoffset_t root, const char *file_identifier, bool size_prefix) {
        ...
        PreAlign(
            (size_prefix ? sizeof(uoffset_t) : 0) + sizeof(uoffset_t) +
