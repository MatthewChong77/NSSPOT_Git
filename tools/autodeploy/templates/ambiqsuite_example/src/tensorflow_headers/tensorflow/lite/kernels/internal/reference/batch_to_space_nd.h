...
inline void BatchToSpaceND(
    ...
    const RuntimeShape &unextended_input2_shape, const int32_t *block_shape_data,
    const RuntimeShape &unextended_input3_shape, const int32_t *crops_data,
    ...
    TFLITE_DCHECK_EQ(
        unextended_input1_shape.DimensionsCount(), unextended_output_shape.DimensionsCount());

    const RuntimeShape input1_shape = ExtendShapeBatchToSpace(unextended_input1_shape);
    const RuntimeShape output_shape = ExtendShapeBatchToSpace(unextended_output_shape);
    ...
    for (int in_batch = 0; in_batch < input_batch_size; ++in_batch) {
        ...
        for (int in_h = 0; in_h < input_height; ++in_h) {
            const int out_h =
                in_h * block_shape_height + spatial_offset / block_shape_width - crops_top;
