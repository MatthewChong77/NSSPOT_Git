...
class RecordingMicroAllocator : public MicroAllocator {
  ...
  protected:
    ...
    // the temp section.
    TfLiteStatus PopulateTfLiteTensorFromFlatbuffer(
        const Model *model, TfLiteTensor *tensor, int tensor_index, int subgraph_index,
        bool allocate_temp) override;

  private:
    RecordingMicroAllocator(
        RecordingSingleArenaBufferAllocator *memory_allocator, MicroMemoryPlanner *memory_planner);
    ...
    void RecordAllocationUsage(
        const RecordedAllocation &snapshotted_allocation, RecordedAllocation &recorded_allocation);
