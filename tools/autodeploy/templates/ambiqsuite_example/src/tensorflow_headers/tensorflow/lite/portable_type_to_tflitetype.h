...
template <typename T> constexpr TfLiteType typeToTfLiteType() { return kTfLiteNoType; }
// Map from TfLiteType to the corresponding C++ type.
// Example:
//   TfLiteTypeToType<kTfLiteBool>::Type -> bool
template <TfLiteType TFLITE_TYPE_ENUM> struct TfLiteTypeToType {}; // Specializations below
...
#define MATCH_TYPE_AND_TFLITE_TYPE(CPP_TYPE, TFLITE_TYPE_ENUM)                                     \
    template <> constexpr TfLiteType typeToTfLiteType<CPP_TYPE>() { return TFLITE_TYPE_ENUM; }     \
    template <> struct TfLiteTypeToType<TFLITE_TYPE_ENUM> { using Type = CPP_TYPE; }
