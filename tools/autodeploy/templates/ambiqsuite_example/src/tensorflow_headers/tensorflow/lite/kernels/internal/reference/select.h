...
void Select(
    const RuntimeShape &input_condition_shape, const D *input_condition_data,
    const RuntimeShape &input_x_shape, const T *input_x_data, const RuntimeShape &input_y_shape,
    const T *input_y_data, const RuntimeShape &output_shape, T *output_data) {
...
void RankOneSelect(
    ...
    if (input_condition_shape.DimensionsCount() == 0) {
        ...
        TFLITE_DCHECK_EQ(
            MatchingDim(input_x_shape, 0, input_y_shape, 0, output_shape, 0), outer_size);
...
void BroadcastSelect5DSlow(
    ...
    const RuntimeShape extended_output_shape = RuntimeShape::ExtendedShape(5, output_shape);
    CopyDimsToDesc(extended_output_shape, &desc_output);
    NdArrayDescsForElementwiseBroadcast(
        input_condition_shape, input_x_shape, input_y_shape, &desc_condition, &desc_x, &desc_y);
