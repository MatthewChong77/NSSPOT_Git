...
class LstmTensors {
  public:
    ...
    const TfLiteTensor *CellStateTensor() const { return internal_tensors_[kLstmCellStateTensor]; }
...
TfLiteStatus ValidateWeightTensorSize(
    TfLiteContext *context, const TfLiteTensor *tensor, int dim1_size, int dim2_size);
...
// configuration
TfLiteStatus ValidateTensorSize(
    TfLiteContext *context, const LstmTensors &tensors, const LstmSizeInfo &size_info);

// Wrapper function to create gate parameters for the four internal LSTM gates
TfLiteStatus CreateGateParams(
    TfLiteContext *context,
    /*Input tensors*/
    const TfLiteTensor *input, const TfLiteTensor *input_weight, const TfLiteTensor *input_bias,
...
tflite::ArithmeticParams CreateInterGateMulParams(
    ...
    const TfLiteType output_type, const int output_zp = 0);
...
TfLiteStatus PrepareGateParametersFloat(
    TfLiteContext *context, const LstmTensors &lstm_tensors, OpDataLSTM *op_data_lstm);
...
LSTMBuffers<CellType> CreateLSTMBuffers(TfLiteContext *context, const int *buffer_indices) {
    ...
    buffers.buffer0 =
        reinterpret_cast<CellType *>(context->GetScratchBuffer(context, buffer_indices[0]));
    buffers.buffer1 =
        reinterpret_cast<CellType *>(context->GetScratchBuffer(context, buffer_indices[1]));
    buffers.buffer2 =
        reinterpret_cast<CellType *>(context->GetScratchBuffer(context, buffer_indices[2]));
    buffers.buffer3 =
        reinterpret_cast<CellType *>(context->GetScratchBuffer(context, buffer_indices[3]));
    return buffers;
}
...
void Tanh(
    int32_t cell_state_scale_power, const RuntimeShape &input_data_shape, int16_t *input_data,
    const RuntimeShape &output_data_shape, int16_t *output_data);

void Tanh(
    int32_t cell_state_scale_power, const RuntimeShape &input_data_shape, float *input_data,
    const RuntimeShape &output_data_shape, float *output_data);

void Mul(
    const RuntimeShape &shape, const ArithmeticParams &params, const int16_t *input1_data,
    const int16_t *input2_data, int8_t *output_data);
...
void Mul(
    const RuntimeShape &shape, const ArithmeticParams &params, const float *input1_data,
    const float *input2_data, float *output_data);

void FullyConnected(
    const FullyConnectedParams &params, const RuntimeShape &input_shape, const int8_t *input_data,
    const RuntimeShape &filter_shape, const int8_t *filter_data, const RuntimeShape &bias_shape,
    const int32_t *bias_data, const RuntimeShape &output_shape, int16_t *output_data);

void FullyConnected(
    const FullyConnectedParams &params, const RuntimeShape &input_shape, const int16_t *input_data,
    const RuntimeShape &filter_shape, const int8_t *filter_data, const RuntimeShape &bias_shape,
    const int64_t *bias_data, const RuntimeShape &output_shape, int16_t *output_data);

void FullyConnected(
    const FullyConnectedParams &params, const RuntimeShape &input_shape, const float *input_data,
    const RuntimeShape &filter_shape, const float *filter_data, const RuntimeShape &bias_shape,
    const float *bias_data, const RuntimeShape &output_shape, float *output_data);

void AddElementWise(
    const int16_t *input_1, const int16_t *input_2, int n_batch, int n_input, int16_t *output);

void AddElementWise(
    const float *input_1, const float *input_2, int n_batch, int n_input, float *output);

void Clipping(const int v_size, const CellStateInfo &cell_state_info, int16_t *vector);

void Clipping(const int v_size, const CellStateInfo &cell_state_info, float *vector);
...
void CalculateLstmGate(
    ...
    TFLITE_DCHECK_LE(
        step_info.InputOffset() + step_info.InputShape().FlatSize(),
    ...
    FullyConnected(
        ...
        tflite::micro::GetTensorData<ActivationType>(input) + step_info.InputOffset(),
        micro::GetTensorShape(input_weight), tflite::micro::GetTensorData<WeightType>(input_weight),
        tflite::micro::GetTensorShape(input_bias),
        tflite::micro::GetOptionalTensorData<BiasType>(input_bias), gate_output_shape, gate_output);

    // Recurrent FC
    FullyConnected(
        gate_params.recurrent_fc_params, step_info.StateShape(),
        tflite::micro::GetTensorData<ActivationType>(recurrent) + step_info.HiddenStateOffset(),
        ...
        tflite::micro::GetOptionalTensorData<BiasType>(recurrent_bias), gate_output_shape,
        fc_output_buffer);

    AddElementWise(
        gate_output, fc_output_buffer,
    ...
    case kTfLiteActTanh: {
        ...
        Tanh(
            /*cell_state_scale_power=*/-12, gate_output_shape, gate_output, gate_output_shape,
            gate_output);
    } break;
    default:
...
template <typename CellType>
void UpdateLstmCell(
    const LstmStepManager &step_info, TfLiteEvalTensor *cell_state,
    // Gate outputs
    CellType *forget_gate_output, const CellType *input_gate_output,
    const CellType *cell_gate_output,
    // Mul parameters
    const ArithmeticParams &forget_cell_mul_params, const ArithmeticParams &input_mul_params,
    ...
    Mul(cell_state_shape, forget_cell_mul_params, forget_gate_output,
        tflite::micro::GetTensorData<CellType>(cell_state) + step_info.CellStateOffset(),
        tflite::micro::GetTensorData<CellType>(cell_state) + step_info.CellStateOffset());
    ...
    AddElementWise(
        tflite::micro::GetTensorData<CellType>(cell_state) + step_info.CellStateOffset(), buffer,
    ...
    if (cell_state_info.cell_clip > 0) {
        Clipping(
            cell_state_shape.FlatSize(), cell_state_info,
            tflite::micro::GetTensorData<CellType>(cell_state) + step_info.CellStateOffset());
    }
}
...
void UpdateLstmHidden(
    const LstmStepManager &step_info, TfLiteEvalTensor *cell_state, TfLiteEvalTensor *hidden_state,
    const CellType *output_gate_output, const ArithmeticParams &mul_params,
    ...
    Mul(cell_state_shape, mul_params, buffer, output_gate_output,
        tflite::micro::GetTensorData<ActivationType>(hidden_state) + step_info.HiddenStateOffset());
}

template <typename ActivationType, typename WeightType, typename CellType, typename BiasType>
void LstmStep(
    const LstmStepManager &step_info, const OpDataLSTM &op_data, LSTMKernelContents &kernel_content,
    ...
    CalculateLstmGate<ActivationType, WeightType, CellType, BiasType>(
        ...
        kernel_content.GetInternalTensor(tflite::kLstmRecurrentToForgetWeightsTensor),
    ...
    CalculateLstmGate<ActivationType, WeightType, CellType, BiasType>(
        ...
        // Recurrent FC
        kernel_content.HiddenStateTensor(),
        kernel_content.GetInternalTensor(tflite::kLstmRecurrentToInputWeightsTensor),
        /*recurrent_bias*/ nullptr,
        // Output
        input_gate_output,
        // Scratch arrays
    ...
    CalculateLstmGate<ActivationType, WeightType, CellType, BiasType>(
        ...
        // Recurrent FC
        kernel_content.HiddenStateTensor(),
        kernel_content.GetInternalTensor(tflite::kLstmRecurrentToCellWeightsTensor),
        /*recurrent_bias*/ nullptr,
        // Output
        cell_gate_output,
        // Scratch arrays
    ...
    UpdateLstmCell<CellType>(
        step_info, kernel_content.CellStateTensor(), forget_gate_output, input_gate_output,
        cell_gate_output, inter_gate_params.forget_cell_mul_params,
        inter_gate_params.input_mul_params, op_data.cell_state_info, updated_input_buffer);
    ...
    CalculateLstmGate<ActivationType, WeightType, CellType, BiasType>(
        ...
        // Recurrent FC
        kernel_content.HiddenStateTensor(),
        kernel_content.GetInternalTensor(tflite::kLstmRecurrentToOutputWeightsTensor),
        /*recurrent_bias*/ nullptr,
        // Output
        output_gate_output,
        // Scratch arrays
    ...
    tflite::lstm_internal::UpdateLstmHidden<CellType, ActivationType>(
        step_info, kernel_content.CellStateTensor(), kernel_content.HiddenStateTensor(),
        output_gate_output, inter_gate_params.output_mul_params,
        op_data.cell_state_info.cell_state_scale_power, tanh_activated_cell_buffer);
    ...
    ActivationType *output_ptr =
        tflite::micro::GetTensorData<ActivationType>(kernel_content.output_tensor);
    const auto *hidden_state = kernel_content.HiddenStateTensor();
    std::memcpy(
        output_ptr + step_info.OutputOffset(),
        tflite::micro::GetTensorData<ActivationType>(hidden_state) + step_info.HiddenStateOffset(),
