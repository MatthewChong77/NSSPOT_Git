...
template <typename T>
void TransposeImpl(
    const int depth, const int dims, const int32_t *perm, const T *input_data,
    const int *input_stride, T *output_data, const int *output_stride,
    ...
    if (depth == dims - 1) {
        ...
        for (int i = 0; i < dimension_size; ++i) {
            TransposeImpl(
                depth + 1, dims, perm, input_data, input_stride, output_data, output_stride,
                output_shape);
...
template <int Size> struct TransposeStorageType;

template <> struct TransposeStorageType<1> { using type = int8_t; };

template <> struct TransposeStorageType<2> { using type = int16_t; };

template <> struct TransposeStorageType<4> { using type = int32_t; };

template <> struct TransposeStorageType<8> { using type = int64_t; };
...
inline void SetupTransposeStrides(
    std::array<int, kTransposeMaxDimensions> &stride, const int32_t *shape, const int dims) {
    stride[dims - 1] = 1;
...
void Transpose(
    const TransposeParams &params, const RuntimeShape &input_shape, const T *input_data,
    ...
    const StorageType *const input_data_storage = reinterpret_cast<const StorageType *>(input_data);
    StorageType *const output_data_storage = reinterpret_cast<StorageType *>(output_data);
    ...
    TransposeImpl(
        0, dims, &params.perm[0], input_data_storage, input_stride.data(), output_data_storage,
