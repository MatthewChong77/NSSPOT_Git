...
template <typename T>
inline void
Add(const ArithmeticParams &params, const RuntimeShape &input1_shape, const T *input1_data,
...
// be fast.
inline void AddScalarBroadcast(
    int size, const ArithmeticParams &params, uint8_t input1_data, const uint8_t *input2_data,
    uint8_t *output_data) {
...
inline void
Add(const ArithmeticParams &params, const RuntimeShape &input1_shape, const uint8_t *input1_data,
...
inline void AddGeneralParamScale(
...
inline void
Add(const ArithmeticParams &params, const RuntimeShape &input1_shape, const int16_t *input1_data,
    const RuntimeShape &input2_shape, const int16_t *input2_data, const RuntimeShape &output_shape,
    int16_t *output_data, bool pot_scale = true) {
    if (!pot_scale) {
        AddGeneralParamScale(
            params, input1_shape, input1_data, input2_shape, input2_data, output_shape,
            output_data);
        return;
    }
...
inline typename std::enable_if<!is_small_integer<T>::value || dummy, void>::type BroadcastAdd6DSlow(
    ...
    const RuntimeShape extended_output_shape = RuntimeShape::ExtendedShape(6, output_shape);
    ...
    for (int a = 0; a < extended_output_shape.Dims(0); ++a) {
        ...
        for (int d = 0; d < extended_output_shape.Dims(1); ++d) {
            ...
            for (int b = 0; b < extended_output_shape.Dims(2); ++b) {
                ...
                output_offset_b += extended_output_shape.Dims(3) * extended_output_shape.Dims(4) *
                                   extended_output_shape.Dims(5);
            }
            input1_offset_d += desc1.strides[1];
            input2_offset_d += desc2.strides[1];
            output_offset_d += extended_output_shape.Dims(2) * extended_output_shape.Dims(3) *
                               extended_output_shape.Dims(4) * extended_output_shape.Dims(5);
        }
        input1_offset_a += desc1.strides[0];
        input2_offset_a += desc2.strides[0];
        output_offset_a += extended_output_shape.Dims(1) * extended_output_shape.Dims(2) *
...
inline void BroadcastAdd4DSlow(
    ...
    const RuntimeShape &input2_shape, const T *input2_data, const RuntimeShape &output_shape,
    T *output_data) {
    return BroadcastAdd6DSlow(
        params, input1_shape, input1_data, input2_shape, input2_data, output_shape, output_data);
}

inline void BroadcastAddFivefold(
    const ArithmeticParams &unswitched_params, const RuntimeShape &unswitched_input1_shape,
    const uint8_t *unswitched_input1_data, const RuntimeShape &unswitched_input2_shape,
    const uint8_t *unswitched_input2_data, const RuntimeShape &output_shape, uint8_t *output_data) {
    ...
    if (y4 > 1) {
            ...
            for (int i1 = 0; i1 < y1; ++i1) {
                ...
                for (int i2 = 0; i2 < y2; ++i2) {
                    for (int i3 = 0; i3 < y3; ++i3) {
                        AddElementwise(
                            y4, params, input1_data_ptr, input2_data_ptr, output_data_ptr);
                        input2_data_ptr += y4;
                        output_data_ptr += y4;
                    }
        ...
        for (int i0 = 0; i0 < y0; ++i0) {
            ...
            for (int i1 = 0; i1 < y1; ++i1) {
                ...
                for (int i2 = 0; i2 < y2; ++i2) {
                    AddScalarBroadcast(
                        y3, params, *input1_data_ptr, input2_data_ptr, output_data_ptr);
                    input2_data_ptr += y3;
                    output_data_ptr += y3;
                    input1_data_ptr += 1;
                }
