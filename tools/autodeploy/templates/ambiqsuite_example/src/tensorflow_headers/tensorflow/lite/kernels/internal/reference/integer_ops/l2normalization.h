...
inline void L2Normalization(
    int32_t input_zero_point, int32_t outer_size, int32_t depth, const int8_t *input_data,
    int8_t *output_data) {
    ...
    for (int outer_index = 0; outer_index < outer_size; ++outer_index) {
        ...
        GetInvSqrtQuantizedMultiplierExp(
            acc, kReverseShift, &inv_l2norm_multiplier, &inv_l2norm_shift);
