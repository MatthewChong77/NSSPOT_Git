...
inline void BroadcastPrelu4DSlow(
    const PreluParams &params, const RuntimeShape &input_shape, const T *input_data,
    const RuntimeShape &alpha_shape, const T *alpha_data, const RuntimeShape &output_shape,
    T *output_data) {
            ...
            for (int x = 0; x < extended_output_shape.Dims(2); ++x) {
                for (int c = 0; c < extended_output_shape.Dims(3); ++c) {
                    ...
                    if (input_value >= 0) {
                        ...
                        const int32_t alpha_value = params.alpha_offset + alpha_data[alpha_index];
...
inline void Prelu(
    ...
    const int flat_size = MatchingElementsSize(input_shape, alpha_shape, output_shape);
