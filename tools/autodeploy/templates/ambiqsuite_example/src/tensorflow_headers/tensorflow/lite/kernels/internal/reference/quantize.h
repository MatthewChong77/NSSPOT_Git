...
inline void AffineQuantize(
    const tflite::QuantizationParams &op_params, const RuntimeShape &input_shape,
    ...
    for (int i = 0; i < flat_size; i++) {
        ...
        int32_t unclamped =
            static_cast<int32_t>(TfLiteRound(val / static_cast<float>(scale))) + zero_point;
...
inline void PerChannelQuantize(
    const tflite::PerChannelQuantizationParams &op_params, const RuntimeShape &input_shape,
    const InputT *input_data, const RuntimeShape &output_shape, OutputT *output_data) {
    ...
    do {
        size_t offset = ReducedOutputOffset(
            num_dims, reinterpret_cast<const int *>(dims_data), current_dim.data(), 0, nullptr);
        const InputT val = input_data[offset];
        const int channel = current_dim[quantized_dimension];
        int32_t unclamped =
            static_cast<int32_t>(TfLiteRound(val / static_cast<float>(scale[channel]))) +
            zero_point[channel];
        int32_t clamped = std::min(std::max(unclamped, min_val), max_val);
