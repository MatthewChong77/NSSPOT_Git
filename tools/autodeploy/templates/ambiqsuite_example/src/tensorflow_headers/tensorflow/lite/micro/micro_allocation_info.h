...
class AllocationInfoBuilder {
  public:
    AllocationInfoBuilder(
        const Model *model, INonPersistentBufferAllocator *non_persistent_allocator)
    ...
    TfLiteStatus GetOfflinePlannedOffsets(const int32_t **offline_planner_offsets);
    ...
    TfLiteStatus
    InitializeAllocationInfo(const int32_t *offline_offsets, SubgraphAllocations *allocations);
    ...
    TfLiteStatus MarkAllocationLifetimes(
        ...
        ScratchBufferHandle *scratch_buffer_handles, SubgraphAllocations *allocations);
  ...
  private:
    ...
    TfLiteStatus ValidateSubgraph(const SubGraph *subgraph, TfLiteEvalTensor *eval_tensors);
