...
inline void Tanh(
    const RuntimeShape &input_shape, const float *input_data, const RuntimeShape &output_shape,
    float *output_data) {
...
inline void Tanh(
    const TanhParams &, const RuntimeShape &input_shape, const float *input_data,
...
inline void Tanh(
    const TanhParams &params, const RuntimeShape &input_shape, const int16_t *input_data,
    ...
    if (input_left_shift == 0) {
        ...
        for (int i = 0; i < flat_size; i++) {
            F3 input = F3::FromRaw(gemmlowp::SaturatingRoundingMultiplyByPOT<1>(input_data[i]));
            F0 output = gemmlowp::tanh(input);
            output_data[i] = output.raw();
        }
    }
...
inline void Tanh(
    const TanhParams &params, const RuntimeShape &input_shape, const uint8_t *input_data,
    ...
    for (int i = 0; i < flat_size; i++) {
        ...
        const int32_t input_val_centered = static_cast<int32_t>(input_val_u8) - input_zero_point;
        uint8_t output_val;
