...
inline bool operator==(const QuantizationParams &qp1, const QuantizationParams &qp2) {
...
inline size_t ReducedOutputOffset(
    const int num_dims, const int *dims, const int *index, const int num_axis, const int *axis) {
    if (num_dims == 0) {
        return 0;
    }
    TFLITE_DCHECK(dims != nullptr);
...
inline int Offset(const Dims<4> &dims, int i0, int i1, int i2, int i3) {
    TFLITE_DCHECK((i0 == 0 && dims.sizes[0] == 0) || (i0 >= 0 && i0 < dims.sizes[0]));
    TFLITE_DCHECK((i1 == 0 && dims.sizes[1] == 0) || (i1 >= 0 && i1 < dims.sizes[1]));
    TFLITE_DCHECK((i2 == 0 && dims.sizes[2] == 0) || (i2 >= 0 && i2 < dims.sizes[2]));
    TFLITE_DCHECK((i3 == 0 && dims.sizes[3] == 0) || (i3 >= 0 && i3 < dims.sizes[3]));
...
int MatchingArraySize(
    const ArrayType1 &array1, int index1, const ArrayType2 &array2, int index2, Args... args) {
    TFLITE_DCHECK_EQ(ArraySize(array1, index1), ArraySize(array2, index2));
...
inline int
MatchingDim(const RuntimeShape &shape1, int index1, const RuntimeShape &shape2, int index2) {
...
int MatchingDim(
    const RuntimeShape &shape1, int index1, const RuntimeShape &shape2, int index2, Args... args) {
    TFLITE_DCHECK_EQ(shape1.Dims(index1), shape2.Dims(index2));
...
inline int RequiredBufferSizeForDims(const Dims<4> &dims) { return FlatSize(dims); }

inline int MatchingElementsSize(const RuntimeShape &shape, const RuntimeShape &check_shape_0) {
...
inline int MatchingElementsSize(
...
// arrays.
inline int MatchingFlatSize(const RuntimeShape &shape, const RuntimeShape &check_shape_0) {
...
inline int MatchingFlatSize(
    const RuntimeShape &shape, const RuntimeShape &check_shape_0, const RuntimeShape &check_shape_1,
...
inline int MatchingFlatSize(
    const RuntimeShape &shape, const RuntimeShape &check_shape_0, const RuntimeShape &check_shape_1,
    const RuntimeShape &check_shape_2, const RuntimeShape &check_shape_3) {
    TFLITE_DCHECK_EQ(shape.DimensionsCount(), check_shape_0.DimensionsCount());
    const int dims_count = shape.DimensionsCount();
    for (int i = 0; i < dims_count; ++i) {
        TFLITE_DCHECK_EQ(shape.Dims(i), check_shape_0.Dims(i));
...
template <int N>
inline int
MatchingFlatSize(const Dims<N> &dims, const Dims<N> &check_dims_0, const Dims<N> &check_dims_1) {
    for (int i = 0; i < N; ++i) {
        TFLITE_DCHECK_EQ(ArraySize(dims, i), ArraySize(check_dims_0, i));
    }
...
template <int N>
inline int MatchingFlatSize(
    const Dims<N> &dims, const Dims<N> &check_dims_0, const Dims<N> &check_dims_1,
...
inline int MatchingFlatSize(
    const Dims<N> &dims, const Dims<N> &check_dims_0, const Dims<N> &check_dims_1,
    const Dims<N> &check_dims_2, const Dims<N> &check_dims_3) {
    for (int i = 0; i < N; ++i) {
        TFLITE_DCHECK_EQ(ArraySize(dims, i), ArraySize(check_dims_0, i));
    }
...
inline int
MatchingExtendedShapeFlatSize(const RuntimeShape &shape, const RuntimeShape &check_shape_0) {
    ...
    for (int i = 0; i < min_dims; ++i) {
        TFLITE_DCHECK_EQ(
            shape.Dims(shape_dims - 1 - i), check_shape_0.Dims(check_shape_0_dims - 1 - i));
    }
...
inline int MatchingExtendedShapeFlatSize(
    const RuntimeShape &shape, const RuntimeShape &check_shape_0,
    const RuntimeShape &check_shape_1) {
    const int flat_size = MatchingExtendedShapeFlatSize(shape, check_shape_0);
    TFLITE_DCHECK_EQ(MatchingExtendedShapeFlatSize(shape, check_shape_1), flat_size);
    return flat_size;
}
...
inline int MatchingExtendedShapeFlatSize(
    ...
    const RuntimeShape &check_shape_2) {
    const int flat_size = MatchingExtendedShapeFlatSize(shape, check_shape_0);
    TFLITE_DCHECK_EQ(MatchingExtendedShapeFlatSize(shape, check_shape_1, check_shape_2), flat_size);
    return flat_size;
}
...
inline int MatchingExtendedShapeFlatSize(
    ...
    const int flat_size = MatchingExtendedShapeFlatSize(shape, check_shape_0);
    TFLITE_DCHECK_EQ(
        MatchingExtendedShapeFlatSize(shape, check_shape_1, check_shape_2, check_shape_3),
        flat_size);
    return flat_size;
}
...
// the depth).
template <int N> inline int FlatSizeSkipDim(const Dims<N> &dims, int skip_dim) {
...
template <int N>
inline int MatchingFlatSizeSkipDim(const Dims<N> &dims, int skip_dim, const Dims<N> &check_dims_0) {
    for (int i = 0; i < N; ++i) {
        if (i != skip_dim) {
            TFLITE_DCHECK_EQ(ArraySize(dims, i), ArraySize(check_dims_0, i));
        }
...
template <int N>
inline int MatchingFlatSizeSkipDim(
    const Dims<N> &dims, int skip_dim, const Dims<N> &check_dims_0, const Dims<N> &check_dims_1) {
    for (int i = 0; i < N; ++i) {
        if (i != skip_dim) {
            TFLITE_DCHECK_EQ(ArraySize(dims, i), ArraySize(check_dims_0, i));
        }
...
inline int MatchingFlatSizeSkipDim(
    ...
    return MatchingFlatSizeSkipDim(dims, skip_dim, check_dims_1, check_dims_2, check_dims_3);
}
...
// A combination of MatchingFlatSize() and FlatSizeSkipDim().
inline int MatchingFlatSizeSkipDim(
    const RuntimeShape &shape, int skip_dim, const RuntimeShape &check_shape_0) {
    const int dims_count = shape.DimensionsCount();
    for (int i = 0; i < dims_count; ++i) {
        if (i != skip_dim) {
            TFLITE_DCHECK_EQ(shape.Dims(i), check_shape_0.Dims(i));
...
inline int MatchingFlatSizeSkipDim(
    ...
    return MatchingFlatSizeSkipDim(shape, skip_dim, check_shape_1, check_shape_2, check_shape_3);
}
...
template <int N> void ComputeStrides(Dims<N> *dims) {
    dims->strides[0] = 1;
    for (int d = 1; d < N; d++) {
...
template <typename P> inline void SetActivationParams(float min, float max, P *params) {
    params->float_activation_min = min;
    params->float_activation_max = max;
}

template <typename P> inline void SetActivationParams(int32_t min, int32_t max, P *params) {
    params->quantized_activation_min = min;
    params->quantized_activation_max = max;
}

template <typename P> inline void SetActivationParams(uint32_t min, uint32_t max, P *params) {
    params->quantized_activation_min = min;
    params->quantized_activation_max = max;
}

template <typename P> inline void SetActivationParams(int16_t min, int16_t max, P *params) {
    params->int16_activation_min = min;
    params->int16_activation_max = max;
}

template <typename P> inline void SetActivationParams(int64_t min, int64_t max, P *params) {
...
template <typename P> inline void GetActivationParams(const P &params, int16_t *min, int16_t *max) {
...
struct is_small_integer
    : public std::integral_constant<
          bool, std::is_same<T, int8_t>::value || std::is_same<T, uint8_t>::value ||
...
struct is_int32_or_int64
    : public std::integral_constant<
          bool, std::is_same<T, int32_t>::value || std::is_same<T, int64_t>::value> {};
