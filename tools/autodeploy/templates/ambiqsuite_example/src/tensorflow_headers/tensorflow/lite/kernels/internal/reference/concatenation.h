...
inline void Concatenation(
    const ConcatenationParams &params, const RuntimeShape *const *input_shapes,
...
inline void ConcatenationWithScaling(
    const ConcatenationParams &params, const RuntimeShape *const *input_shapes,
    const uint8_t *const *input_data, const RuntimeShape &output_shape, uint8_t *output_data) {
    int axis = params.axis;
            ...
            if (input_zeropoint[i] == output_zeropoint && input_scale[i] == output_scale) {
                ...
                for (int j = 0; j < copy_size; ++j) {
                    const int32_t value =
                        ...
                        output_zeropoint;
                    output_ptr[j] =
                        static_cast<uint8_t>(std::max<int32_t>(std::min<int32_t>(255, value), 0));
                }
            }
            output_ptr += copy_size;
        }
