...
struct LSTMKernelContents {
  public:
    ...
    TfLiteEvalTensor *HiddenStateTensor() { return internal_tensors[kLstmOutputStateTensor]; }
    TfLiteEvalTensor *CellStateTensor() { return internal_tensors[kLstmCellStateTensor]; }
    // Node internal tensors with indexes defined at the beginning of the file
    TfLiteEvalTensor *internal_tensors[24];
    TfLiteEvalTensor *output_tensor;
};

template <typename CellType> struct LSTMBuffers {
