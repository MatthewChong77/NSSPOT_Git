...
inline void LogSoftmax(
    const SoftmaxParams &params, const RuntimeShape &input_shape, const float *input_data,
...
inline void LogSoftmax(
    const SoftmaxParams &params, const RuntimeShape &input_shape, const uint8_t *input_data,
    ...
    for (int i = 0; i < outer_size; ++i) {
        ...
        const int32_t fixed_log_sum_of_exps =
            log_x_for_x_greater_than_or_equal_to_1<kScaledDiffIntegerBits>(sum_of_exps).raw();
        ...
        const int adjusted_diff_min = std::max(
            static_cast<int32_t>(diff_min - 1), // Note use of > below instead of >= above.
            MultiplyByQuantizedMultiplierSmallerThanOneExp(
                rescaled_diff_min, reverse_scaling_divisor, -reverse_scaling_right_shift));
        ...
        for (int c = 0; c < depth; ++c) {
            ...
            if (input_diff > adjusted_diff_min) {
                ...
                output_data[i * depth + c] = static_cast<uint8_t>(std::max(
                    std::min(unsat_output, static_cast<int32_t>(255)), static_cast<int32_t>(0)));
            } else {
...
template <typename T>
inline void LogSoftmaxQuantized(
    const SoftmaxParams &params, const size_t outer_size, const size_t depth,
    const RuntimeShape &input_shape, const T *input_data, const RuntimeShape &output_shape,
    T *output_data) {
    const int32_t input_multiplier = params.input_multiplier;
    const int32_t input_left_shift = params.input_left_shift;
    const int32_t reverse_scaling_divisor = params.reverse_scaling_divisor;
    ...
    for (size_t outer_index = 0; outer_index < outer_size; ++outer_index) {
        ...
        for (size_t inner_index = 0; inner_index < depth; ++inner_index) {
            int32_t input_diff =
                static_cast<int32_t>(input_data[outer_index * depth + inner_index]) - max_in_row;
            if (input_diff >= diff_min) {
                const int32_t input_diff_in_q5 =
                    MultiplyByQuantizedMultiplier(input_diff, input_multiplier, input_left_shift);
                sum_of_exps_in_q12 =
                    sum_of_exps_in_q12 + gemmlowp::Rescale<kAccumulationIntegerBits>(
                                             exp_on_negative_values(F5::FromRaw(input_diff_in_q5)));
            }
        }

        const int32_t log_sum_of_exps_in_q5 =
            log_x_for_x_greater_than_or_equal_to_1<kInputIntegerBits>(sum_of_exps_in_q12).raw();
        ...
        const int32_t shifted_log_sum_of_exps_in_q5 = log_sum_of_exps_in_q5 + kMinInt32;
        const int32_t adjusted_diff_min = std::max(
            static_cast<int32_t>(diff_min - 1),
            MultiplyByQuantizedMultiplier(
                shifted_log_sum_of_exps_in_q5, reverse_scaling_divisor,
        ...
        for (size_t inner_index = 0; inner_index < depth; ++inner_index) {
            ...
            if (input_diff > adjusted_diff_min) {
                ...
                output_in_q27 = std::max(
                    std::min(output_in_q27, static_cast<int32_t>(kMaxT8)),
                    static_cast<int32_t>(kMinT8));
                output_data[outer_index * depth + inner_index] = static_cast<T>(output_in_q27);
            } else {
...
inline void LogSoftmax(
    const SoftmaxParams &params, const size_t outer_size, const size_t depth,
    const RuntimeShape &input_shape, const int8_t *input_data, const RuntimeShape &output_shape,
    int8_t *output_data) {
    LogSoftmaxQuantized(
        params, outer_size, depth, input_shape, input_data, output_shape, output_data);
}
