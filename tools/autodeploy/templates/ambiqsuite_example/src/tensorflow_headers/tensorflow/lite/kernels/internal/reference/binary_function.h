...
inline void BroadcastBinaryFunction4DSlow(
    ...
    const RuntimeShape &unextended_output_shape, R *output_data, R (*func)(T1, T2)) {
    ...
    const int *dims_data = reinterpret_cast<const int *>(output_shape.DimsDataUpTo5D());
...
// R: Result type. T1: Input 1 type. T2: Input 2 type.
template <typename R, typename T1, typename T2>
inline void BinaryFunction(
    const RuntimeShape &input1_shape, const T1 *input1_data, const RuntimeShape &input2_shape,
    const T2 *input2_data, const RuntimeShape &output_shape, R *output_data, R (*func)(T1, T2)) {
