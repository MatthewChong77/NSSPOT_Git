...
class MicroMemoryPlanner {
  public:
    ...
    // GetOffsetForBuffer().
    virtual TfLiteStatus AddBuffer(int size, int first_time_used, int last_time_used) = 0;
    ...
    AddBuffer(int size, int first_time_used, int last_time_used, int offline_offset) {
        return kTfLiteError;
    }
    ...
    virtual TfLiteStatus Init(unsigned char *scratch_buffer, int scratch_buffer_size) {
        return kTfLiteOk;
    }
