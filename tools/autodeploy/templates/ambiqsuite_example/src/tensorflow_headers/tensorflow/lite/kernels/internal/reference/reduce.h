...
// in axis.
inline bool IsFirstReduction(const int *index, const int num_axis, const int *axis) {
    if (num_axis == 0) {
        return true;
    }
...
inline bool Reduce(
    const In *input_data, const int *input_dims, const int *output_dims, const int input_num_dims,
    const int output_num_dims, const int *axis, const int num_axis, int *input_iter,
...
inline bool Reduce(
    ...
    const std::function<Out(Out current, In in)> &reducer_next, Out *output_data) {
    // Reset input iterator.
    for (int idx = 0; idx < input_num_dims; ++idx) {
        input_iter[idx] = 0;
    }
...
inline bool ResolveAxis(
    const int num_dims, const int *axis, const int64_t num_axis, int *out_axis, int *out_num_axis) {
...
inline bool ReduceSumImpl(
    ...
    return Reduce<In, Out>(
        input_data, input_dims, output_dims, input_num_dims, output_num_dims, axis, num_axis,
        input_iter, reducer, output_data);
}

template <typename T>
inline bool
InitTensorDataForReduce(const int *dims, const int num_dims, const T init_value, T *data) {
    size_t num_elements = 1;
    for (int idx = 0; idx < num_dims; ++idx) {
        size_t current = static_cast<size_t>(dims[idx]);
        // Overflow prevention.
        if (current > 0 && num_elements > std::numeric_limits<size_t>::max() / current) {
            return false;
        }
...
inline bool ReduceGeneric(
    const T *input_data, const int *input_dims, const int input_num_dims, T *output_data,
    const int *output_dims, const int output_num_dims, const int *axis,
    const int64_t num_axis_dimensions, bool keep_dims, int *temp_index, int *resolved_axis,
    T init_value, T reducer(const T current, const T in)) {
    // Reset output data.
    if (!InitTensorDataForReduce(output_dims, output_num_dims, init_value, output_data)) {
        return false;
    }
    ...
    if (!ResolveAxis(
            input_num_dims, axis, num_axis_dimensions, resolved_axis, &num_resolved_axis)) {
        return false;
    }

    return Reduce<T, T>(
        input_data, input_dims, output_dims, input_num_dims, output_num_dims, resolved_axis,
...
inline void Mean(
    const tflite::MeanParams &op_params, const RuntimeShape &unextended_input_shape,
    const float *input_data, const RuntimeShape &unextended_output_shape, float *output_data) {
...
inline bool QuantizedMeanOrSum(
    const T *input_data, int32_t input_zero_point, const int *input_dims, const int input_num_dims,
    T *output_data, int32_t output_multiplier, int output_shift, int32_t output_zero_point,
    const int *output_dims, const int output_num_dims, const int *axis,
    const int num_axis_dimensions, bool keep_dims, int *temp_index, int *resolved_axis, U *temp_sum,
    bool compute_sum) {
    ...
    for (int idx = 0; idx < num_resolved_axis; ++idx) {
        ...
        // Overflow prevention.
        if (current >
            static_cast<size_t>(std::numeric_limits<int64_t>::max() / num_elements_in_axis)) {
            return false;
        }
        num_elements_in_axis *= current;
    }
    ...
    if (!compute_sum) {
        ...
        output_multiplier = static_cast<int32_t>(
            (static_cast<int64_t>(output_multiplier) << shift) / num_elements_in_axis);
    ...
    for (size_t idx = 0; idx < num_outputs; ++idx) {
        ...
        int32_t output =
            MultiplyByQuantizedMultiplier(shifted_sum, output_multiplier, output_shift) +
            output_zero_point;
...
inline bool QuantizedMeanOrSumExtraArgs(
    const T *input_data, int32_t input_zero_point, float input_scale, const int *input_dims,
    const int input_num_dims, T *output_data, float output_scale, int32_t output_multiplier,
    int output_shift, int32_t output_zero_point, const int *output_dims, const int output_num_dims,
    const int *axis, const int num_axis_dimensions, bool keep_dims, int *temp_index,
    int *resolved_axis, U *temp_sum, bool compute_sum) {
    return QuantizedMeanOrSum<T, U>(
        input_data, input_zero_point, input_dims, input_num_dims, output_data, output_multiplier,
        output_shift, output_zero_point, output_dims, output_num_dims, axis, num_axis_dimensions,
        keep_dims, temp_index, resolved_axis, temp_sum, compute_sum);
}

template <typename T>
inline bool QuantizedReduceProd(
    const T *input_data, int32_t input_zero_point, const RuntimeShape &input_shape, T *output_data,
    int32_t output_zero_point, const RuntimeShape &output_shape, const int *axis,
    const int64_t num_axis_dimensions, bool keep_dims, int *temp_index, int *resolved_axis,
    ...
    int num_resolved_axis = 0;
    if (!ResolveAxis(
            input_shape.DimensionsCount(), axis, num_axis_dimensions, resolved_axis,
            &num_resolved_axis)) {
        return false;
    }
    ...
    auto reducer_next = [&](int32_t current, T in) -> int32_t {
        const int64_t result = static_cast<int64_t>(current) * (in - input_zero_point);
    ...
    if (!Reduce<T, int32_t>(
            ...
            input_shape.DimensionsCount(), output_shape.DimensionsCount(), resolved_axis,
            num_resolved_axis, temp_index, reducer_first, reducer_next, temp_prod)) {
        return false;
    }
    ...
    for (int i = 0; i < output_shape.FlatSize(); i++) {
        int32_t result =
            MultiplyByQuantizedMultiplier(
                static_cast<int64_t>(temp_prod[i]), scaling_multiplier, scaling_shift) +
            output_zero_point;
