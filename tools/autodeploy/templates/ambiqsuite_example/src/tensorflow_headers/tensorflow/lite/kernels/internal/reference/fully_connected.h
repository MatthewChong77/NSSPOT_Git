...
inline void FullyConnected(
    ...
    const RuntimeShape &weights_shape, const float *weights_data, const RuntimeShape &bias_shape,
...
inline void FullyConnected(
    const FullyConnectedParams &params, const RuntimeShape &input_shape, const uint8_t *input_data,
    ...
    for (int b = 0; b < batches; ++b) {
        for (int out_c = 0; out_c < output_depth; ++out_c) {
            ...
            for (int d = 0; d < accum_depth; ++d) {
                ...
                int16_t filter_val = filter_data[out_c * accum_depth + d] + filter_offset;
...
inline void ShuffledFullyConnected(
    const FullyConnectedParams &params, const RuntimeShape &input_shape, const uint8_t *input_data,
    const RuntimeShape &weights_shape, const uint8_t *shuffled_weights_data,
    ...
    const int batches = FlatSizeSkipDim(output_shape, output_dim_count - 1);
    const int output_depth =
        MatchingDim(weights_shape, weights_dim_count - 2, output_shape, output_dim_count - 1);
