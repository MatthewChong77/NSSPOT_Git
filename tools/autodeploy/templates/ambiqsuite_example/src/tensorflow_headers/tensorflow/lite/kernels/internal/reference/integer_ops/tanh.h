...
inline void Tanh(
    ...
    int32_t input_shift, const RuntimeShape &input_shape, const int8_t *input_data,
    ...
    for (int i = 0; i < flat_size; ++i) {
        ...
        if (input <= -input_range_radius) {
            ...
            const int32_t output_in_q0 = gemmlowp::tanh(F4::FromRaw(input_in_q4)).raw();
...
inline void Tanh(
    int32_t input_multiplier, int32_t input_left_shift, const RuntimeShape &input_shape,
    const int16_t *ptr_input_data, const RuntimeShape &output_shape, int16_t *ptr_output_data) {
