...
inline void Requantize(
    const input_type *input_data, int32_t size, int32_t effective_scale_multiplier,
    ...
    const bool same_scale = (effective_scale_multiplier == 1 << 30 && effective_scale_shift == 1);
    if (same_scale) {
        const bool mixed_type_int8_uint8 =
            std::is_same<input_type, int8_t>::value && std::is_same<output_type, uint8_t>::value;
        const bool mixed_type_uint8_int8 =
            std::is_same<input_type, uint8_t>::value && std::is_same<output_type, int8_t>::value;
