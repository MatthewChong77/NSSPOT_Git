...
tflite::FullyConnectedParams CreateFCParams(
    ...
    const double input_product_scale = input_quant_params.scale * weight_quant_params.scale;
    double effective_scale =
        input_product_scale / static_cast<double>(nonlinear_activation_input_scale);

    QuantizeMultiplier(effective_scale, &data.output_multiplier, &data.output_shift);
...
inline tflite::FullyConnectedParams CreateFCParamsFloat() {
    ...
    CalculateActivationRange(
        kTfLiteActNone, &op_params.float_activation_min, &op_params.float_activation_max);
    return op_params;
}
...
tflite::ArithmeticParams CreateInterGateMulParams(
    const float input1_scale, const float input2_scale, const float output_scale,
    ...
    QuantizeMultiplier(effective_scale, &op_params.output_multiplier, &op_params.output_shift);
    return op_params;
}
...
// pointers
template <
    typename ActivationType, typename WeightType, typename BiasType, typename CellType,
    int batch_size, int time_steps, int input_dimension, int state_dimension>
LSTMKernelContents
CreateLSTMKernelContent(LstmNodeContent<
                        ActivationType, WeightType, BiasType, CellType, batch_size, time_steps,
    ...
    kernel_content.internal_tensors[kLstmInputLayerNormCoefficientsTensor] = nullptr;
    kernel_content.internal_tensors[kLstmForgetLayerNormCoefficientsTensor] = nullptr;
    kernel_content.internal_tensors[kLstmInputLayerNormCoefficientsTensor] = nullptr;
    kernel_content.internal_tensors[kLstmCellLayerNormCoefficientsTensor] = nullptr;
    kernel_content.internal_tensors[kLstmOutputLayerNormCoefficientsTensor] = nullptr;
    // Output tensor
...
LstmSizeInfo CreateLstmSizeInfo(
    ...
    size_info.batch_size = time_major ? input_tensor_shape->data[1] : input_tensor_shape->data[0];
    size_info.time_steps = time_major ? input_tensor_shape->data[0] : input_tensor_shape->data[1];
...
template <
    typename ActivationType, typename WeightType, typename BiasType, typename CellType,
    int batch_size, int time_steps, int input_dimension, int state_dimension>
OpDataLSTM CreateLstmOpData(LstmNodeContent<
                            ActivationType, WeightType, BiasType, CellType, batch_size, time_steps,
                            input_dimension, state_dimension> &node_contents) {
    ...
    op_data.size_info = CreateLstmSizeInfo(
        builtin_data.time_major, node_contents.GetEvalTensor(kLstmInputTensor)->dims,
        node_contents.HiddenStateEvalTensor()->dims);

    op_data.cell_state_info =
        CreateLstmCellStateInfo(quantization_settings.cell_state.scale, builtin_data.cell_clip);

    // Gate Parameters
    op_data.forget_gate_parameters = CreateGateParams<CellType>(
        quantization_settings.input, quantization_settings.hidden_state,
        quantization_settings.forget_gate, quantization_settings.nonlinear_activation_input_scale);
    op_data.input_gate_parameters = CreateGateParams<CellType>(
        quantization_settings.input, quantization_settings.hidden_state,
        quantization_settings.input_gate, quantization_settings.nonlinear_activation_input_scale);
    op_data.cell_gate_parameters = CreateGateParams<CellType>(
        quantization_settings.input, quantization_settings.hidden_state,
        quantization_settings.cell_gate, quantization_settings.nonlinear_activation_input_scale);
    op_data.output_gate_parameters = CreateGateParams<CellType>(
        quantization_settings.input, quantization_settings.hidden_state,
        quantization_settings.output_gate, quantization_settings.nonlinear_activation_input_scale);
    // Inter gate multiplication parameters
    op_data.inter_gate_parameters.forget_cell_mul_params = CreateInterGateMulParams<CellType>(
        quantization_settings.nonlinear_activation_output_scale,
        quantization_settings.cell_state.scale, quantization_settings.cell_state.scale);
    op_data.inter_gate_parameters.input_mul_params = CreateInterGateMulParams<CellType>(
        quantization_settings.nonlinear_activation_output_scale,
        quantization_settings.nonlinear_activation_output_scale,
        quantization_settings.cell_state.scale);
    op_data.inter_gate_parameters.output_mul_params = CreateInterGateMulParams<ActivationType>(
        quantization_settings.nonlinear_activation_output_scale,
        quantization_settings.nonlinear_activation_output_scale,
        quantization_settings.hidden_state.scale, quantization_settings.hidden_state.zero_point);
    return op_data;
}

template <int batch_size, int time_steps, int input_dimension, int state_dimension>
OpDataLSTM CreateLstmOpDataFloat(LstmNodeContent<
                                 float, float, float, float, batch_size, time_steps,
                                 input_dimension, state_dimension> &node_contents) {
    const auto &builtin_data = node_contents.BuiltinData();
    OpDataLSTM op_data;
    ...
    // Inter gate multiplication parameters
    op_data.inter_gate_parameters.forget_cell_mul_params = CreateInterGateMulParamsFloat();
    op_data.inter_gate_parameters.input_mul_params = CreateInterGateMulParamsFloat();
    op_data.inter_gate_parameters.output_mul_params = CreateInterGateMulParamsFloat();
    return op_data;
}

/*Test Functions Below Here*/
template <typename T>
void ValidateResultGoldens(
    const T *golden, const T *output_data, const int output_len, const float tolerance) {
...
void TestCalculateLstmGateFloat(
    ...
    // Result comparison
    TfLiteFusedActivation nonlinear_type, const float *expected_vals, float tolerance) {
    ...
    ValidateResultGoldens(expected_vals, gate_output, batch_size * state_dimension, tolerance);
}
...
void TestCalculateLstmGateInteger(
    ...
    tflite::GateParameters gate_params = CreateGateParams<CellType>(
        ...
        gate_quantization_settings, node_quantization_settings.nonlinear_activation_input_scale);
    ...
    tflite::lstm_internal::CalculateLstmGate<ActivationType, WeightType, CellType, BiasType>(
        step_info, gate_params,
        // Input FC
        input, input_weight, input_bias,
        // Recurrent FC
    ...
    Dequantize(
        gate_output, batch_size * state_dimension,
        node_quantization_settings.nonlinear_activation_output_scale, 0, gate_output_float);

    ValidateResultGoldens(
        expected_vals, gate_output_float, batch_size * state_dimension, tolerance);
}

template <int batch_size, int time_steps, int input_dimension, int state_dimension>
void TestUpdateLstmCellFloat(
    const GateOutputCheckData<batch_size * input_dimension, batch_size * state_dimension>
        &gate_output_data,
    LstmNodeContent<
        float, float, float, float, batch_size, time_steps, input_dimension, state_dimension>
        &node_content,
    ...
    // Create step information: only one time step, no need to update
    auto size_info = tflite::testing::CreateLstmSizeInfo(
        /*time_major*/ false, node_content.GetEvalTensor(tflite::kLstmInputTensor)->dims,
    ...
    std::memcpy(
        forget_gate, gate_output_data.expected_forget_gate_output,
    ...
    tflite::lstm_internal::UpdateLstmCell<float>(
        step_info, cell_state, forget_gate, gate_output_data.expected_input_gate_output,
        gate_output_data.expected_cell_gate_output, forget_cell_mul_params, input_mul_params,
        cell_state_info, buffer);

    ValidateResultGoldens(
        gate_output_data.expected_updated_cell, tflite::micro::GetTensorData<float>(cell_state),
...
void TestUpdateLstmCellInteger(
    const GateOutputCheckData<batch_size * input_dimension, batch_size * state_dimension>
        &gate_output_data,
    LstmNodeContent<
        ActivationType, WeightType, BiasType, CellType, batch_size, time_steps, input_dimension,
        state_dimension> &node_content,
    const float tolerance) {
    ...
    tflite::Quantize(
        gate_output_data.expected_forget_gate_output, quantized_forget_gate,
        batch_size * state_dimension, quantization_settings.nonlinear_activation_output_scale, 0);

    CellType quantized_input_gate[batch_size * state_dimension] = {};
    tflite::Quantize(
        gate_output_data.expected_input_gate_output, quantized_input_gate,
        batch_size * state_dimension, quantization_settings.nonlinear_activation_output_scale, 0);

    CellType quantized_cell_gate[batch_size * state_dimension] = {};
    tflite::Quantize(
        gate_output_data.expected_cell_gate_output, quantized_cell_gate,
        batch_size * state_dimension, quantization_settings.nonlinear_activation_output_scale, 0);
    ...
    auto cell_state_info = CreateLstmCellStateInfo(
        quantization_settings.cell_state.scale, node_content.BuiltinData().cell_clip);
    ...
    tflite::lstm_internal::UpdateLstmCell<CellType>(
        step_info, cell_state, quantized_forget_gate, quantized_input_gate, quantized_cell_gate,
        forget_cell_mul_params, input_mul_params, cell_state_info, buffer);

    float cell_state_float[batch_size * state_dimension] = {};
    Dequantize(
        tflite::micro::GetTensorData<CellType>(cell_state), batch_size * state_dimension,
        quantization_settings.cell_state.scale, quantization_settings.cell_state.zero_point,
        cell_state_float);

    ValidateResultGoldens(
        gate_output_data.expected_updated_cell, cell_state_float, batch_size * state_dimension,
        tolerance);
}
...
void TestUpdateLstmHiddenFloat(
    ...
    tflite::lstm_internal::UpdateLstmHidden<float, float>(
        step_info, cell_state, hidden_state, gate_output_data.expected_output_gate_output,
        mul_params, cell_state_scale_power, buffer);

    ValidateResultGoldens(
        gate_output_data.expected_updated_hidden, tflite::micro::GetTensorData<float>(hidden_state),
        batch_size * state_dimension, tolerance);
}
...
void TestUpdateLstmHiddenInteger(
    ...
    tflite::Quantize(
        gate_output_data.expected_output_gate_output, quantized_output_gate,
        batch_size * state_dimension, quantization_settings.nonlinear_activation_output_scale, 0);
    ...
    tflite::CheckedLog2(quantization_settings.cell_state.scale, &cell_state_scale_power_buffer);
    ...
    Dequantize(
        tflite::micro::GetTensorData<ActivationType>(hidden_state), batch_size * state_dimension,
        quantization_settings.hidden_state.scale, quantization_settings.hidden_state.zero_point,
        hidden_state_float);

    ValidateResultGoldens(
        gate_output_data.expected_updated_hidden, hidden_state_float, batch_size * state_dimension,
        tolerance);
}
...
void TestLstmStepInteger(
    ...
    op_data.size_info.time_major = true;
    tflite::lstm_internal::LstmStepManager step_info(&op_data.size_info);
    tflite::lstm_internal::LstmStep<ActivationType, WeightType, CellType, BiasType>(
        step_info, op_data, kernel_content, buffers);

    const auto &quantization_settings = node_contents.QuantizationSettings();
    float dequantized_hidden_state[batch_size * state_dimension] = {};
    Dequantize(
        tflite::micro::GetTensorData<ActivationType>(kernel_content.HiddenStateTensor()),
    ...
    ValidateResultGoldens(
        gate_output_data.expected_updated_hidden, dequantized_hidden_state,
        batch_size * state_dimension, hidden_state_tolerance);
    ValidateResultGoldens(
        gate_output_data.expected_updated_cell, dequantized_cell_state,
        batch_size * state_dimension, cell_state_tolerance);
}
...
void TestEvalLstmFloat(
    ...
    tflite::EvalLstm<float, float, float, float>(op_data, kernel_content, buffers);

    ValidateResultGoldens(
        eval_check_data.expected_hidden_state, node_contents.GetHiddenStateData(),
        batch_size * state_dimension, hidden_state_tolerance);

    ValidateResultGoldens(
        eval_check_data.expected_cell_state, node_contents.GetCellStateData(),
        batch_size * state_dimension, cell_state_tolerance);

    ValidateResultGoldens(
        eval_check_data.expected_output, node_contents.GetOutputData(),
        batch_size * state_dimension, hidden_state_tolerance);
}
...
void TestEvalLstmInteger(
    ...
    float dequantized_hidden_state[batch_size * state_dimension] = {};
    Dequantize(
        node_contents.GetHiddenStateData(), batch_size * state_dimension,
        quantization_settings.hidden_state.scale, quantization_settings.hidden_state.zero_point,
        dequantized_hidden_state);

    ValidateResultGoldens(
        eval_check_data.expected_hidden_state, dequantized_hidden_state,
        batch_size * state_dimension, hidden_state_tolerance);

    float dequantized_cell_state[batch_size * state_dimension] = {};
    Dequantize(
        node_contents.GetCellStateData(), batch_size * state_dimension,
        quantization_settings.cell_state.scale, quantization_settings.cell_state.zero_point,
        dequantized_cell_state);
    ValidateResultGoldens(
        eval_check_data.expected_cell_state, dequantized_cell_state, batch_size * state_dimension,
        cell_state_tolerance);

    float dequantized_output[batch_size * state_dimension * time_steps] = {};
    Dequantize(
        node_contents.GetOutputData(), batch_size * state_dimension * time_steps,
        quantization_settings.output.scale, quantization_settings.output.zero_point,
        dequantized_output);
    ValidateResultGoldens(
        eval_check_data.expected_output, dequantized_output, batch_size * state_dimension,
